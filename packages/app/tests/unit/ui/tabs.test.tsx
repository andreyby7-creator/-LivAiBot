...
vi.mock('@livai/ui-core/components/Tabs', async () => {
...
describe('App Tabs', () => {
  ...
  describe('I18n рендеринг', () => {
    describe('Aria-label', () => {
      it('должен рендерить обычный aria-label', () => {
        ...
        expect(screen.getByTestId('core-tabs')).toHaveAttribute('aria-label', 'Test label');
      });
      ...
      it('должен рендерить i18n aria-label', () => {
        render(
          <Tabs
            items={testItems}
            visible
            {...{ ariaLabelI18nKey: 'navigation.tabs' } as any}
          />,
        );

        expect(mockTranslate).toHaveBeenCalledWith('common', 'navigation.tabs', {});
        expect(screen.getByTestId('core-tabs')).toHaveAttribute('aria-label', 'Translated Label');
      });
      ...
      it('должен передавать namespace для i18n aria-label', () => {
        render(
          <Tabs
            items={testItems}
            visible
            {...{ ariaLabelI18nKey: 'tabs', ariaLabelI18nNs: 'navigation' } as any}
          />,
        );

        expect(mockTranslate).toHaveBeenCalledWith('navigation', 'tabs', {});
      });

      it('должен передавать параметры для i18n aria-label', () => {
        const params = { count: 3, section: 'settings' };
        render(
          <Tabs
            items={testItems}
            visible
            {...{ ariaLabelI18nKey: 'navigation.tabs', ariaLabelI18nParams: params } as any}
          />,
        );

        expect(mockTranslate).toHaveBeenCalledWith('common', 'navigation.tabs', params);
      });
      ...
      it('должен использовать пустой объект для undefined параметров i18n aria-label', () => {
        render(
          <Tabs
            items={testItems}
            visible
            {...{ ariaLabelI18nKey: 'navigation.tabs', ariaLabelI18nParams: undefined } as any}
          />,
        );
    ...
    describe('Aria-labelledby', () => {
      it('должен рендерить обычный aria-labelledby', () => {
        ...
        expect(screen.getByTestId('core-tabs')).toHaveAttribute('aria-labelledby', 'test-id');
      });
      ...
      it('должен передавать параметры для i18n aria-labelledby', () => {
        const params = { id: 'main-nav', type: 'tabs' };
        render(
          <Tabs
            items={testItems}
            visible
  ...
  describe('Discriminated union типизация', () => {
    it('должен принимать обычный aria-label без i18n', () => {
      ...
      expect(screen.getByTestId('core-tabs')).toHaveAttribute('aria-label', 'Regular label');
    });
    ...
    it('должен принимать обычный aria-labelledby без i18n', () => {
      ...
      expect(screen.getByTestId('core-tabs')).toHaveAttribute('aria-labelledby', 'regular-id');
    });
