...
vi.mock('@livai/ui-core/primitives/divider', async () => {
...
describe('App Divider', () => {
  ...
  describe('I18n рендеринг', () => {
    describe('Aria-label', () => {
      it('должен рендерить обычный aria-label', () => {
        render(<Divider aria-label='Test label' />);

        expect(screen.getByTestId('core-divider')).toHaveAttribute('aria-label', 'Test label');
      });

      it('должен рендерить i18n aria-label', () => {
        render(<Divider {...{ ariaLabelI18nKey: 'common.label' } as any} />);

        expect(mockTranslate).toHaveBeenCalledWith('common', 'common.label', {});
        expect(screen.getByTestId('core-divider')).toHaveAttribute(
          'aria-label',
          'Translated Label',
        );
      });
      ...
      it('должен передавать namespace для i18n aria-label', () => {
        render(<Divider {...{ ariaLabelI18nKey: 'auth.login', ariaLabelI18nNs: 'auth' } as any} />);
      ...
      it('должен передавать параметры для i18n aria-label', () => {
        ...
        render(
          <Divider {...{ ariaLabelI18nKey: 'common.field', ariaLabelI18nParams: params } as any} />,
        );
  ...
  describe('Побочные эффекты и производительность', () => {
    it('должен мемоизировать i18n aria-label при изменении пропсов', () => {
      const { rerender } = render(<Divider {...{ ariaLabelI18nKey: 'common.first' } as any} />);

      expect(mockTranslate).toHaveBeenCalledTimes(1);

      rerender(<Divider {...{ ariaLabelI18nKey: 'common.second' } as any} />);
  ...
  describe('Discriminated union типизация', () => {
    it('должен принимать обычный aria-label без i18n', () => {
      render(<Divider aria-label='Regular label' />);

      expect(screen.getByTestId('core-divider')).toHaveAttribute('aria-label', 'Regular label');
    });

    it('должен принимать i18n aria-label без обычного', () => {
      render(<Divider {...{ ariaLabelI18nKey: 'common.test' } as any} />);
