...
import { ERROR_CODE, assertNever, isErrorCode } from '../../../../src/errors/base/ErrorCode.ts'
import {
  ERROR_SEVERITY,
  ERROR_CATEGORY,
  ERROR_ORIGIN,
...
describe('Error Kernel - Golden ABI Tests', () => {
  describe('ERROR_CODE stability', () => {
    ...
    it('should maintain immutability', () => {
      ...
      ;((): void => { ERROR_CODE.NEW_CODE = 'NEW_CODE' })()
  ...
  describe('Error immutability', () => {
    it('should be deeply frozen (runtime + type-level immutability)', () => {
      ...
      const attemptCodeMutation = (): void => { ;(error as any).code = 'OTHER_CODE' }
      const attemptMessageMutation = (): void => { ;(error as any).message = 'New message' }
      const attemptNestedMutation = (): void => { ;(error.context as any).nested = {} }
    ...
    it('should demonstrate runtime immutability protection', () => {
      ...
      expect(() => { ;(error as any).code = 'OTHER_CODE' }).toThrow(TypeError)
  ...
  describe('wrapUnknownError', () => {
    ...
    it('должен обрабатывать null/undefined', () => {
      ...
      const wrappedUndefined = wrapUnknownError(undefined, ERROR_CODE.UNKNOWN_ERROR, 'Fallback message')
  ...
  describe('Error chaining (cause)', () => {
    ...
    it('should handle BaseError chain with different error codes', () => {
      ...
      expect(((infraError.cause as BaseError).cause as BaseError).code).toBe(ERROR_CODE.DOMAIN_ENTITY_NOT_FOUND)
    })
  })
