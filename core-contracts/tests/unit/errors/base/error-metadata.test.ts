...
describe('ErrorMetadata', () => {
  describe('createErrorMetadata', () => {
    ...
    it('should handle null in context and extra', () => {
      ...
      expect((metadata.context as { field: null; anotherField: string; })?.anotherField).toBe(
        'value',
      );
      expect(metadata.extra).toBeDefined();
    });
    ...
    it('should handle nested objects in context', () => {
      ...
      expect(metadata.context).toBeDefined();
      const user = (metadata.context as { user: { id: string; name: string; }; operation: string; })
        ?.user;
      expect(user?.id).toBe('user-123');
      expect(user?.name).toBe('John');
      expect(
        (metadata.context as { user: { id: string; name: string; }; operation: string; })
          ?.operation,
      ).toBe('create');
    });
