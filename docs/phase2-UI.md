...
2️⃣2️⃣4️⃣ effects/login/login-audit.mapper.ts 🔴 — ts — deps: domain/AuthAuditEvent, domain/LoginRequest, domain/LoginResult, effects/login/login-effect.types, schemas; (pure mapper для преобразования результатов login-flow в AuthAuditEvent: mapLoginResultToAuditEvent принимает DomainLoginResult/LoginSecurityResult/LoginRequest/LoginMetadata и формирует валидированный AuthAuditEvent через auditEventSchema, фиксирует login_success только после успешного /me, login_failure/login_blocked для ошибок и блокировок, fail-closed через assertNever, изолирует audit-структуру от login.ts, используется через DI LoginEffectDeps.auditLogger)
2️⃣2️⃣5️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-utils, domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/login-api.mapper, effects/login/login-effect.types, effects/login/login-metadata.enricher, effects/login/login-store-updater, effects/login/login-audit.mapper; (тонкий orchestrator login-flow: validate-input (strict Zod loginRequestSchema.strict()) → security-pipeline (через SecurityPipelinePort, deterministic decision: block short-circuit до API call) → enrich-metadata (buildLoginMetadata с валидированным LoginContext) → двухфазный API-call (/v1/auth/login + /v1/auth/me через validatedEffect с loginTokenPairSchema/meResponseSchema, fail-closed: без успешного /me → login не считается успешным) → domain mapping (mapLoginResponseToDomain) → update-store (через login-store-updater: updateLoginState/applyBlockedState, атомарный batchUpdate) → audit logging (через login-audit.mapper и injected auditLogger, login_success только после успешного /me), hierarchical timeout (step timeouts + global hard timeout через withTimeout), concurrency control (cancel_previous/ignore/serialize), все ошибки через injected errorMapper.map с sanitized messages, инварианты: нет бизнес-логики, нет прямых Date.now(), все side-effects через DI-порты)
