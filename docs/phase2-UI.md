...
2️⃣2️⃣7️⃣ effects/shared/safe-record.ts 🔴 — ts — deps: — ; (безопасная работа с Record: isPlainObject, validateSafeRecordPayload, freezeShallowRecord, только shallow freeze)
2️⃣2️⃣8️⃣ types/auth-initial.ts 🔴 — ts — deps: types/auth; (canonical initial states: INITIAL_AUTH_STATE, INITIAL_SESSION_STATE, INITIAL_SECURITY_STATE)
...
2️⃣3️⃣4️⃣ lib/session-manager.ts 🔴 — ts — deps: types/auth, @livai/core/policies/AuthPolicy, domain/SessionPolicy; (auto-refresh, expiry, session policies)
2️⃣3️⃣5️⃣ effects/logout/logout-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth; (DI: LogoutEffectDeps, LogoutEffectConfig, LogoutMode local/remote)
2️⃣3️⃣6️⃣ effects/logout/logout-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, types/auth-initial; (applyLogoutReset: атомарный reset через batchUpdate, использует INITIAL_* states)
2️⃣3️⃣7️⃣ effects/logout.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, store-utils), stores/auth; (logout orchestrator, safeSet, setStoreLocked)
2️⃣3️⃣8️⃣ effects/register/register-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RegisterEffectDeps, RegisterEffectConfig, без securityPipeline)
2️⃣3️⃣9️⃣ effects/register/register-api.mapper.ts 🔴 — ts — deps: domain/RegisterRequest, domain/RegisterResponse, schemas, effects/shared/auth-api.mappers, effects/shared/safe-record; (mapper: RegisterRequest→RegisterRequestValues, RegisterResponseDto→domain, pure functions)
2️⃣4️⃣0️⃣ effects/register/register-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRegisterSuccess: использует session-state.builder, fail-closed)
2️⃣4️⃣1️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-utils), domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, lib/error-mapper, effects/register/_; (register orchestrator: validate→api→map→store)
2️⃣4️⃣2️⃣ effects/refresh/refresh-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RefreshEffectDeps, RefreshEffectConfig, isolation guard обязателен)
2️⃣4️⃣3️⃣ effects/refresh/refresh-api.mapper.ts 🔴 — ts — deps: domain/RefreshTokenRequest, domain/TokenPair, domain/MeResponse, schemas, effects/shared/auth-api.mappers; (mapper: mapRefreshRequestToApiPayload, mapRefreshResponseToDomain, использует shared mappers)
2️⃣4️⃣4️⃣ effects/refresh/refresh-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRefreshSuccess: использует session-state.builder, fail-closed при частично успешном refresh)
2️⃣4️⃣5️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-isolation, store-utils), types/auth, @livai/core/policies/AuthPolicy, schemas, effects/refresh/_; (refresh token, idempotency guard, использует AuthStorePort через DI)
2️⃣4️⃣6️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (React adapter: authState, login, logout, refresh)
