...
2️⃣0️⃣8️⃣ types/auth.ts 🟢 — ts — deps: @livai/domains/policies, domain/_; (AuthState, AuthStatus, AuthError, MFA/OAuth/Security/Recovery types)
...
2️⃣1️⃣1️⃣ lib/risk-assessment.adapter.ts 🟢 — ts — deps: @livai/domains/strategies, ipaddr.js, domain/DeviceInfo, domain/LoginRiskAssessment, types/auth; (adapter: ClassificationRule→RiskReason, DeviceInfo→DeviceRiskInfo, createLoginRiskEvaluation)
2️⃣1️⃣2️⃣ lib/risk-assessment.ts 🟢 — ts — deps: @livai/domains (aggregation, policies, signals, strategies), lib/classification-mapper, lib/risk-assessment.adapter, domain/DeviceInfo, domain/LoginRiskAssessment, types/auth-risk; (composition: assessClassification→auth decision, plugin pattern)
...
2️⃣2️⃣2️⃣ effects/login/login-api.mapper.ts 🟢 — ts — deps: domain/LoginRequest, domain/LoginResult, domain/MeResponse, domain/MfaChallengeRequest, domain/TokenPair, schemas, types/login.dto; (mapper: LoginRequest→LoginRequestValues, LoginResponseDto→DomainLoginResult, assertNever)
...
2️⃣2️⃣5️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, orchestrator, schema-validated-effect), domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/_; (orchestrator: validate→security→enrich→api(/login+/me)→map→store→audit, timeout, DI)
...
2️⃣3️⃣3️⃣ effects/shared/auth-store.port.ts 🔴 — ts — deps: types/auth; (AuthStorePort: единый контракт стора для всех auth-эффектов, batchUpdate для атомарности)
2️⃣3️⃣4️⃣ effects/shared/session-state.builder.ts 🔴 — ts — deps: types/auth, domain/TokenPair, domain/MeResponse, domain/DeviceInfo; (централизованное построение SessionState: проверка issuedAt<=expiresAt, copy-on-write, freeze)
2️⃣3️⃣5️⃣ effects/shared/auth-api.mappers.ts 🔴 — ts — deps: schemas, domain/TokenPair, domain/MeResponse; (общие мэпперы: mapTokenPairValuesToDomain, mapMeResponseValuesToDomain)
2️⃣3️⃣6️⃣ effects/shared/safe-record.ts 🔴 — ts — deps: — ; (безопасная работа с Record: isPlainObject, validateSafeRecordPayload, freezeShallowRecord, только shallow freeze)
2️⃣3️⃣7️⃣ lib/session-manager.ts 🔴 — ts — deps: types/auth, @livai/core/policies/AuthPolicy, domain/SessionPolicy; (auto-refresh, expiry, session policies)
...
2️⃣3️⃣9️⃣ effects/logout/logout-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth; (DI: LogoutEffectDeps, LogoutEffectConfig, LogoutMode local/remote)
2️⃣4️⃣0️⃣ effects/logout/logout-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, types/auth-initial; (applyLogoutReset: атомарный reset через batchUpdate, использует INITIAL_* states)
2️⃣4️⃣1️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-utils), domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, lib/error-mapper, effects/register/*; (register orchestrator: validate→api→map→store)
2️⃣4️⃣2️⃣ effects/register/register-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RegisterEffectDeps, RegisterEffectConfig, без securityPipeline)
2️⃣4️⃣3️⃣ effects/register/register-api.mapper.ts 🔴 — ts — deps: domain/RegisterRequest, domain/RegisterResponse, schemas, effects/shared/auth-api.mappers, effects/shared/safe-record; (mapper: RegisterRequest→RegisterRequestValues, RegisterResponseDto→domain, pure functions)
2️⃣4️⃣4️⃣ effects/register/register-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRegisterSuccess: использует session-state.builder, fail-closed)
2️⃣4️⃣5️⃣ effects/refresh/refresh-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RefreshEffectDeps, RefreshEffectConfig, isolation guard обязателен)
2️⃣4️⃣6️⃣ effects/refresh/refresh-api.mapper.ts 🔴 — ts — deps: domain/RefreshTokenRequest, domain/TokenPair, domain/MeResponse, schemas, effects/shared/auth-api.mappers; (mapper: mapRefreshRequestToApiPayload, mapRefreshResponseToDomain, использует shared mappers)
2️⃣4️⃣7️⃣ effects/refresh/refresh-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRefreshSuccess: использует session-state.builder, fail-closed при частично успешном refresh)
2️⃣4️⃣8️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-isolation, store-utils), types/auth, @livai/core/policies/AuthPolicy, schemas, effects/refresh/*; (refresh token, idempotency guard, использует AuthStorePort через DI)
2️⃣4️⃣9️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (React adapter: authState, login, logout, refresh)
