...
2️⃣2️⃣5️⃣ effects/login/login-effect.types.ts 🟢 — ts — deps: @livai/app/lib/effect-utils, effects/shared/auth-store.port, lib/security-pipeline, schemas, types/auth, types/auth-risk; (DI: ApiClient, AuthStorePort, SecurityPipelinePort, LoginEffectDeps)
2️⃣2️⃣6️⃣ effects/login/login-api.mapper.ts 🟢 — ts — deps: domain/LoginRequest, domain/LoginResult, domain/MeResponse, domain/MfaChallengeRequest, domain/TokenPair, schemas, types/login.dto; (mapper: LoginRequest→LoginRequestValues, LoginResponseDto→DomainLoginResult, assertNever)
2️⃣2️⃣7️⃣ effects/login/login-store-updater.ts 🟢 — ts — deps: effects/shared/auth-store.port, effects/login/login-metadata.enricher, domain/LoginResult, lib/security-pipeline, types/auth-risk, types/auth; (applySuccessState/applyMfaState/applyBlockedState, batchUpdate через AuthStorePort)
2️⃣2️⃣8️⃣ effects/login/login-audit.mapper.ts 🟢 — ts — deps: domain/LoginResult, schemas, types/auth; (mapLoginResultToAuditEvent→AuditEventValues, login_success/mfa_challenge/policy_violation/login_failure)
2️⃣2️⃣9️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, orchestrator, schema-validated-effect), domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/_; (orchestrator: validate→security→enrich→api(/login+/me)→map→store→audit, timeout, DI)
2️⃣3️⃣0️⃣ types/auth-initial.ts 🔴 — ts — deps: types/auth; (canonical initial states: INITIAL_AUTH_STATE, INITIAL_SESSION_STATE, INITIAL_SECURITY_STATE)
2️⃣3️⃣1️⃣ effects/shared/session-state.builder.ts 🔴 — ts — deps: types/auth, domain/TokenPair, domain/MeResponse, domain/DeviceInfo; (централизованное построение SessionState: проверка issuedAt<=expiresAt, copy-on-write, freeze)
2️⃣3️⃣2️⃣ effects/shared/auth-api.mappers.ts 🔴 — ts — deps: schemas, domain/TokenPair, domain/MeResponse; (общие мэпперы: mapTokenPairValuesToDomain, mapMeResponseValuesToDomain)
2️⃣3️⃣3️⃣ effects/shared/safe-record.ts 🔴 — ts — deps: — ; (безопасная работа с Record: isPlainObject, validateSafeRecordPayload, freezeShallowRecord, только shallow freeze)
...
2️⃣3️⃣5️⃣ effects/logout.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, store-utils), stores/auth; (logout orchestrator, safeSet, setStoreLocked)
2️⃣3️⃣6️⃣ effects/logout/logout-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth; (DI: LogoutEffectDeps, LogoutEffectConfig, LogoutMode local/remote)
...
2️⃣3️⃣9️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-utils), domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, lib/error-mapper, effects/register/_; (register orchestrator: validate→api→map→store)
2️⃣4️⃣0️⃣ effects/register/register-api.mapper.ts 🔴 — ts — deps: domain/RegisterRequest, domain/RegisterResponse, schemas, effects/shared/auth-api.mappers, effects/shared/safe-record; (mapper: RegisterRequest→RegisterRequestValues, RegisterResponseDto→domain, pure functions)
