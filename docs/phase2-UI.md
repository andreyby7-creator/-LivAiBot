...
Feature Auth
1️⃣8️⃣2️⃣ domain/MfaInfo.ts 🟢 — ts — deps: — ; (DTO MFA: discriminated union totp/sms/email/push, security-aware)
1️⃣8️⃣3️⃣ domain/ClientContext.ts 🟢 — ts — deps: — ; (DTO клиентского окружения: ip, deviceId, userAgent, locale, timezone, geo, sessionId, appVersion, boundary DTO)
1️⃣8️⃣4️⃣ domain/TokenPair.ts 🟢 — ts — deps: — ; (DTO токенов: accessToken, refreshToken, expiresAt)
1️⃣8️⃣5️⃣ domain/LoginRequest.ts 🟢 — ts — deps: domain/MfaInfo, domain/ClientContext; (DTO login: identifier, password, mfa, clientContext)
1️⃣8️⃣6️⃣ domain/RegisterRequest.ts 🟢 — ts — deps: domain/MfaInfo, domain/ClientContext; (DTO register: identifier, username, password, mfa)
1️⃣8️⃣7️⃣ domain/RegisterResponse.ts 🟢 — ts — deps: domain/MfaInfo, domain/TokenPair; (DTO ответа регистрации: userId, tokenPair, mfaChallenge)
1️⃣8️⃣8️⃣ domain/PasswordResetRequest.ts 🟢 — ts — deps: — ; (DTO сброса пароля: identifier, clientContext)
1️⃣8️⃣9️⃣ domain/PasswordResetConfirm.ts 🟢 — ts — deps: — ; (DTO подтверждения сброса: token, newPassword)
1️⃣9️⃣0️⃣ domain/VerifyEmailRequest.ts 🟢 — ts — deps: — ; (DTO верификации email: token)
1️⃣9️⃣1️⃣ domain/VerifyPhoneRequest.ts 🟢 — ts — deps: — ; (DTO верификации телефона: phone, code)
1️⃣9️⃣2️⃣ domain/RefreshTokenRequest.ts 🟢 — ts — deps: — ; (DTO refresh: refreshToken, clientContext)
1️⃣9️⃣3️⃣ domain/LogoutRequest.ts 🟢 — ts — deps: — ; (DTO logout: refreshToken?, clientContext?)
1️⃣9️⃣4️⃣ domain/DeviceInfo.ts 🟢 — ts — deps: — ; (DTO устройства: deviceId, deviceType, os, browser, geo)
1️⃣9️⃣5️⃣ domain/SessionRevokeRequest.ts 🟢 — ts — deps: — ; (DTO отзыва сессии: sessionId, reason)
1️⃣9️⃣6️⃣ domain/MfaChallengeRequest.ts 🟢 — ts — deps: — ; (DTO MFA challenge: userId, type, deviceId)
1️⃣9️⃣7️⃣ domain/MfaSetupRequest.ts 🟢 — ts — deps: — ; (DTO настройки MFA: userId, type, secret)
1️⃣9️⃣8️⃣ domain/MfaBackupCodeRequest.ts 🟢 — ts — deps: — ; (DTO backup MFA: userId, backupCode)
1️⃣9️⃣9️⃣ domain/OAuthLoginRequest.ts 🟢 — ts — deps: — ; (DTO OAuth login: provider, providerToken)
2️⃣0️⃣0️⃣ domain/OAuthRegisterRequest.ts 🟢 — ts — deps: — ; (DTO OAuth register: provider, providerToken, email)
2️⃣0️⃣1️⃣ domain/LoginRiskAssessment.ts 🟢 — ts — deps: @livai/domains/policies; (DTO риска: LoginRiskResult, LoginRiskContext, RiskLevel, deriveLoginDecision)
2️⃣0️⃣2️⃣ domain/SessionPolicy.ts 🟢 — ts — deps: — ; (DTO политик сессии: maxConcurrentSessions, ipPolicy, geoPolicy)
2️⃣0️⃣3️⃣ domain/AuthAuditEvent.ts 🟢 — ts — deps: — ; (DTO аудита: eventId, type, userId, ip, deviceId, geo)
2️⃣0️⃣4️⃣ domain/EmailTemplateRequest.ts 🟢 — ts — deps: — ; (DTO email-шаблонов: templateId, to, variables)
2️⃣0️⃣5️⃣ domain/SmsTemplateRequest.ts 🟢 — ts — deps: — ; (DTO SMS-шаблонов: templateId, to, variables)
2️⃣0️⃣6️⃣ domain/MfaRecoveryRequest.ts 🟢 — ts — deps: — ; (DTO MFA recovery: userId, method, proof)
2️⃣0️⃣7️⃣ domain/OAuthErrorResponse.ts 🟢 — ts — deps: — ; (DTO OAuth ошибок: error, provider, message)
2️⃣0️⃣8️⃣ domain/AuthErrorResponse.ts 🟢 — ts — deps: — ; (DTO auth ошибок: error, message, retryable)
2️⃣0️⃣9️⃣ domain/MeResponse.ts 🟢 — ts — deps: — ; (DTO /me: user, roles, permissions, session)
2️⃣1️⃣0️⃣ schemas/schemas.ts 🟢 — ts+zod — deps: @livai/core-contracts; (Zod схемы auth DTO: login/register/MFA/OAuth/audit/templates, generatedAuth)
2️⃣1️⃣1️⃣ types/auth.ts 🟢 — ts — deps: @livai/domains/policies, domain/_; (AuthState, AuthStatus, AuthError, MFA/OAuth/Security/Recovery types)
2️⃣1️⃣2️⃣ types/auth-risk.ts 🟢 — ts — deps: @livai/domains (aggregation, policies, signals, strategies), domain/LoginRiskAssessment; (RiskContext, RiskPolicy, RiskAssessmentResult, re-export RiskLevel/ClassificationSignals)
2️⃣1️⃣3️⃣ types/login.dto.ts 🟢 — ts — deps: schemas; (LoginResponseDto: discriminated union success/mfa_required из LoginTokenPairValues, MeResponseValues)
2️⃣1️⃣4️⃣ lib/classification-mapper.ts 🟢 — ts — deps: @livai/domains (labels, policies, strategies); (classification labels → auth decision, strategy pattern, pre-filtering)
2️⃣1️⃣5️⃣ lib/device-fingerprint.ts 🟢 — ts+effect — deps: domain/DeviceInfo; (pure effect: userAgent, platform, screen, timezone → DeviceInfo)
2️⃣1️⃣6️⃣ lib/risk-assessment.adapter.ts 🟢 — ts — deps: @livai/domains/strategies, ipaddr.js, domain/DeviceInfo, domain/LoginRiskAssessment, types/auth; (adapter: ClassificationRule→RiskReason, DeviceInfo→DeviceRiskInfo, createLoginRiskEvaluation)
2️⃣1️⃣7️⃣ lib/risk-assessment.ts 🟢 — ts — deps: @livai/domains (aggregation, policies, signals, strategies), lib/classification-mapper, lib/risk-assessment.adapter, domain/DeviceInfo, domain/LoginRiskAssessment, types/auth-risk; (composition: assessClassification→auth decision, plugin pattern)
2️⃣1️⃣8️⃣ stores/auth.ts 🟢 — ts+zustand — deps: types/auth; (Auth store: state+sync transitions, persist, no effects)
2️⃣1️⃣9️⃣ effects/login/validation.ts 🟢 — ts — deps: domain/LoginRequest; (type guards LoginRequest, strict shape validation)
2️⃣2️⃣0️⃣ effects/login/login-metadata.enricher.ts 🟢 — ts — deps: @livai/core, @livai/domains, domain/DeviceInfo, domain/LoginRequest; (ContextEnricher: buildLoginMetadata, PII hash via injection)
2️⃣2️⃣1️⃣ lib/error-mapper.ts 🟢 — ts — deps: @livai/app/lib/error-mapping, domain/AuthErrorResponse, domain/MfaChallengeRequest, domain/OAuthErrorResponse, domain/SessionRevokeRequest, types/auth; (API errors → AuthError, rule-engine, sanitization)
2️⃣2️⃣2️⃣ lib/security-pipeline.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, effect-utils, orchestrator), lib/device-fingerprint, lib/risk-assessment, domain/DeviceInfo, domain/LoginRiskAssessment, types/auth, types/auth-risk; (facade: fingerprint→risk, orchestrator+timeout, fail-closed)
2️⃣2️⃣3️⃣ domain/LoginResult.ts 🟢 — ts — deps: domain/TokenPair, domain/MeResponse, domain/MfaChallengeRequest; (DomainLoginResult: immutable union success/mfa_required)
2️⃣2️⃣4️⃣ effects/shared/auth-store.port.ts 🟢 — ts — deps: stores/auth, types/auth; (AuthStorePort: единый контракт стора для всех auth-эффектов, batchUpdate для атомарности, setStoreLocked для защиты от race conditions)
2️⃣2️⃣5️⃣ effects/shared/api-client.port.ts 🟢 — ts — deps: @livai/app/lib/effect-utils; (AuthApiClientPort: единый контракт HTTP-клиента для всех auth-эффектов, Effect-based, AbortSignal через параметр Effect)
2️⃣2️⃣6️⃣ effects/shared/session-state.builder.ts 🟢 — ts — deps: domain/DeviceInfo, domain/MeResponse, domain/TokenPair, types/auth; (централизованное построение SessionState: проверка issuedAt<=expiresAt через Date.parse, fail-fast на missing fields, copy-on-write, shallow freeze)
2️⃣2️⃣7️⃣ effects/shared/auth-api.mappers.ts 🟢 — ts — deps: domain/MeResponse, domain/TokenPair, schemas; (общие мэпперы: mapTokenPairValuesToDomain, mapMeResponseValuesToDomain, copy-on-write, immutability, safety boundary)
2️⃣2️⃣8️⃣ effects/shared/api-client.adapter.ts 🟢 — ts — deps: @livai/app/lib/effect-utils, effects/shared/api-client.port; (адаптер: Promise-based HTTP-клиент → Effect-based AuthApiClientPort, AbortSignal propagation, fail-closed)
2️⃣2️⃣9️⃣ effects/login/login-effect.types.ts 🟢 — ts — deps: @livai/app/lib/effect-utils, effects/shared/auth-store.port, lib/security-pipeline, schemas, types/auth, types/auth-risk; (DI: ApiClient, AuthStorePort, SecurityPipelinePort, LoginEffectDeps)
2️⃣3️⃣0️⃣ effects/login/login-api.mapper.ts 🟢 — ts — deps: domain/LoginRequest, domain/LoginResult, domain/MfaChallengeRequest, domain/MfaInfo, effects/shared/auth-api.mappers, schemas, types/login.dto; (mapper: LoginRequest→LoginRequestValues, LoginResponseDto→DomainLoginResult, assertNever, использует shared мэпперы для TokenPair и MeResponse)
2️⃣3️⃣1️⃣ effects/login/login-store-updater.ts 🟢 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, effects/login/login-metadata.enricher, domain/LoginResult, lib/security-pipeline, types/auth-risk; (applySuccessState/applyMfaState/applyBlockedState, batchUpdate через AuthStorePort, использует buildSessionState для консистентности)
2️⃣3️⃣2️⃣ effects/login/login-audit.mapper.ts 🟢 — ts — deps: domain/LoginResult, schemas, types/auth; (mapLoginResultToAuditEvent→AuditEventValues, login_success/mfa_challenge/policy_violation/login_failure)
2️⃣3️⃣3️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, orchestrator, schema-validated-effect), domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/_; (orchestrator: validate→security→enrich→api(/login+/me)→map→store→audit, timeout, DI)
2️⃣3️⃣4️⃣ types/auth-initial.ts 🟢 — ts — deps: types/auth, types/auth-risk; (canonical initial states: initialAuthState, createInitialSessionState(), initialSecurityState, единый источник истины для reset, immutable константы и pure функции)
2️⃣3️⃣5️⃣ lib/session-manager.ts 🟢 — ts — deps: types/auth, @livai/core (AuthPolicyConfig), @livai/core-contracts (UnixTimestampMs), domain/SessionPolicy; (domain-pure service: isExpired, shouldRefresh, getRefreshDeadline, shouldInvalidate, canOpenNewSession, proactive refresh, TTL validation, clock skew protection)
2️⃣3️⃣6️⃣ effects/logout/logout-effect.types.ts 🟢 — ts — deps: effects/shared/auth-store.port, effects/shared/api-client.port, lib/security-pipeline, types/auth, types/auth-risk; (DI: LogoutEffectDeps, LogoutEffectConfig, LogoutMode local/remote, discriminated unions, LogoutSecurityDecision, LogoutSecurityResult, validateLogoutConfig, type guards)
2️⃣3️⃣7️⃣ effects/logout/logout-store-updater.ts 🟢 — ts — deps: effects/shared/auth-store.port, types/auth-initial; (applyLogoutReset: атомарный reset через batchUpdate, использует канонические initial states, константа logoutResetActions для расширения, опциональный context для audit/telemetry)
2️⃣3️⃣8️⃣ effects/logout/logout-audit.mapper.ts 🟢 — ts — deps: schemas, types/auth; (mapLogoutResultToAuditEvent→AuditEventValues, mapRevokeErrorToAuditEvent→AuditEventValues, logout_success/logout_failure/revoke_error, fail-closed через auditEventSchema)
2️⃣3️⃣9️⃣ effects/logout.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, effect-utils), types/auth, effects/logout/logout-audit.mapper, effects/logout/logout-effect.types, effects/logout/logout-store-updater, effects/shared/auth-store.port; (logout orchestrator: lock→reset→unlock атомарно, revoke API параллельно после unlock для remote mode, audit logging, concurrency control ignore/cancel_previous/serialize с DoS-защитой, idempotency через batchUpdate, fail-closed, все через DI-порты)
2️⃣4️⃣0️⃣ effects/register/register-effect.types.ts 🟢 — ts — deps: schemas, types/auth, effects/login/login-effect.types, effects/shared/api-client.port, effects/shared/auth-store.port; (DI: RegisterEffectDeps, RegisterEffectConfig, deterministic ports)
2️⃣4️⃣1️⃣ effects/register/register-api.mapper.ts 🟢 — ts — deps: domain/OAuthRegisterRequest, domain/RegisterRequest, domain/RegisterResponse, schemas, effects/shared/auth-api.mappers; (mapper: RegisterRequest→payload, OAuth transport input→payload, RegisterResponseDto→domain, guards+assertNever)
2️⃣4️⃣2️⃣ effects/register/register-store-updater.ts 🟢 — ts — deps: domain/DeviceInfo, domain/MeResponse, domain/RegisterRequest, domain/RegisterResponse, domain/TokenPair, types/auth, types/auth-initial, effects/shared/auth-store.port, effects/shared/session-state.builder; (batchUpdate sink, registry handlers, deterministic fail-closed messages)
2️⃣4️⃣3️⃣ effects/register/register-audit.mapper.ts 🟢 — ts — deps: zod, domain/DeviceInfo, domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth; (mapRegisterResultToAuditEvent→AuditEventValues, oauth_register_success/failure + mfa_challenge, fail-closed через auditEventSchema)
2️⃣4️⃣4️⃣ effects/register/register-metadata.enricher.ts 🟢 — ts — deps: domain/RegisterRequest; (deterministic metadata v='1': trace + identifierHash + timestamp, fail-closed validation)
2️⃣4️⃣5️⃣ effects/register.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, effect-utils, schema-validated-effect), domain/AuthErrorResponse, domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, effects/register/*; (register orchestrator: validate→metadata→api→map→store→audit, concurrency strategies, best-effort telemetry)
2️⃣4️⃣6️⃣ effects/refresh/refresh-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RefreshEffectDeps, RefreshEffectConfig, isolation guard обязателен)
2️⃣4️⃣7️⃣ effects/refresh/refresh-api.mapper.ts 🔴 — ts — deps: domain/RefreshTokenRequest, domain/TokenPair, domain/MeResponse, schemas, effects/shared/auth-api.mappers; (mapper: mapRefreshRequestToApiPayload, mapRefreshResponseToDomain, использует shared mappers)
2️⃣4️⃣8️⃣ effects/refresh/refresh-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRefreshSuccess: использует session-state.builder, fail-closed при частично успешном refresh)
2️⃣4️⃣9️⃣ effects/refresh/refresh-audit.mapper.ts 🔴 — ts — deps: domain/TokenPair, schemas, types/auth; (mapRefreshResultToAuditEvent→AuditEventValues, session_refreshed/refresh_failed/session_invalidated, fail-closed через auditEventSchema)
2️⃣5️⃣0️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-isolation, store-utils), types/auth, @livai/core/policies/AuthPolicy, schemas, effects/refresh/_; (refresh token, idempotency guard, использует AuthStorePort через DI)
2️⃣5️⃣1️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (React adapter: authState, login, logout, refresh)
