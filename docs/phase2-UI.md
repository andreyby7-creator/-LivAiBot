...
2️⃣2️⃣5️⃣ effects/shared/session-state.builder.ts 🟢 — ts — deps: domain/DeviceInfo, domain/MeResponse, domain/TokenPair, types/auth; (централизованное построение SessionState: проверка issuedAt<=expiresAt через Date.parse, fail-fast на missing fields, copy-on-write, shallow freeze)
2️⃣2️⃣6️⃣ effects/shared/auth-api.mappers.ts 🟢 — ts — deps: domain/MeResponse, domain/TokenPair, schemas; (общие мэпперы: mapTokenPairValuesToDomain, mapMeResponseValuesToDomain, copy-on-write, immutability, safety boundary)
2️⃣2️⃣7️⃣ effects/login/login-effect.types.ts 🟢 — ts — deps: @livai/app/lib/effect-utils, effects/shared/auth-store.port, lib/security-pipeline, schemas, types/auth, types/auth-risk; (DI: ApiClient, AuthStorePort, SecurityPipelinePort, LoginEffectDeps)
2️⃣2️⃣8️⃣ effects/login/login-api.mapper.ts 🟢 — ts — deps: domain/LoginRequest, domain/LoginResult, domain/MfaChallengeRequest, domain/MfaInfo, effects/shared/auth-api.mappers, schemas, types/login.dto; (mapper: LoginRequest→LoginRequestValues, LoginResponseDto→DomainLoginResult, assertNever, использует shared мэпперы для TokenPair и MeResponse)
2️⃣2️⃣9️⃣ effects/login/login-store-updater.ts 🟢 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, effects/login/login-metadata.enricher, domain/LoginResult, lib/security-pipeline, types/auth-risk; (applySuccessState/applyMfaState/applyBlockedState, batchUpdate через AuthStorePort, использует buildSessionState для консистентности)
2️⃣3️⃣0️⃣ effects/login/login-audit.mapper.ts 🟢 — ts — deps: domain/LoginResult, schemas, types/auth; (mapLoginResultToAuditEvent→AuditEventValues, login_success/mfa_challenge/policy_violation/login_failure)
2️⃣3️⃣1️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app (effect-timeout, orchestrator, schema-validated-effect), domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/_; (orchestrator: validate→security→enrich→api(/login+/me)→map→store→audit, timeout, DI)
2️⃣3️⃣2️⃣ types/auth-initial.ts 🟢 — ts — deps: types/auth, types/auth-risk; (canonical initial states: initialAuthState, createInitialSessionState(), initialSecurityState, единый источник истины для reset, immutable константы и pure функции)
2️⃣3️⃣3️⃣ lib/session-manager.ts 🟢 — ts — deps: types/auth, @livai/core (AuthPolicyConfig), @livai/core-contracts (UnixTimestampMs), domain/SessionPolicy; (domain-pure service: isExpired, shouldRefresh, getRefreshDeadline, shouldInvalidate, canOpenNewSession, proactive refresh, TTL validation, clock skew protection)
2️⃣3️⃣4️⃣ effects/logout/logout-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth; (DI: LogoutEffectDeps, LogoutEffectConfig, LogoutMode local/remote)
2️⃣3️⃣5️⃣ effects/logout/logout-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, types/auth-initial; (applyLogoutReset: атомарный reset через batchUpdate, использует INITIAL_* states)
2️⃣3️⃣6️⃣ effects/logout.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, store-utils), stores/auth; (logout orchestrator, safeSet, setStoreLocked)
2️⃣3️⃣7️⃣ effects/register/register-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RegisterEffectDeps, RegisterEffectConfig, без securityPipeline)
2️⃣3️⃣8️⃣ effects/register/register-api.mapper.ts 🔴 — ts — deps: domain/RegisterRequest, domain/RegisterResponse, schemas, effects/shared/auth-api.mappers; (mapper: RegisterRequest→RegisterRequestValues, RegisterResponseDto→domain, pure functions)
2️⃣3️⃣9️⃣ effects/register/register-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRegisterSuccess: использует session-state.builder, fail-closed)
2️⃣4️⃣0️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-utils), domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, lib/error-mapper, effects/register/_; (register orchestrator: validate→api→map→store)
2️⃣4️⃣1️⃣ effects/refresh/refresh-effect.types.ts 🔴 — ts — deps: effects/shared/auth-store.port, @livai/app/lib/effect-utils, types/auth, lib/error-mapper; (DI: RefreshEffectDeps, RefreshEffectConfig, isolation guard обязателен)
2️⃣4️⃣2️⃣ effects/refresh/refresh-api.mapper.ts 🔴 — ts — deps: domain/RefreshTokenRequest, domain/TokenPair, domain/MeResponse, schemas, effects/shared/auth-api.mappers; (mapper: mapRefreshRequestToApiPayload, mapRefreshResponseToDomain, использует shared mappers)
2️⃣4️⃣3️⃣ effects/refresh/refresh-store-updater.ts 🔴 — ts — deps: effects/shared/auth-store.port, effects/shared/session-state.builder, types/auth; (applyRefreshSuccess: использует session-state.builder, fail-closed при частично успешном refresh)
2️⃣4️⃣4️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app (orchestrator, schema-validated-effect, effect-timeout, effect-isolation, store-utils), types/auth, @livai/core/policies/AuthPolicy, schemas, effects/refresh/_; (refresh token, idempotency guard, использует AuthStorePort через DI)
2️⃣4️⃣5️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (React adapter: authState, login, logout, refresh)
