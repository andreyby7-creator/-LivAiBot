...
3️⃣1️⃣ packages/ui-core/src/components/Tabs.tsx 🔴 — ts+react — Tabs UI
...
6️⃣4️⃣ packages/app/src/ui/modal.tsx 🟢 — ts+react — UI wrapper Modal — ui-core/Modal.tsx
6️⃣5️⃣ packages/app/src/ui/breadcrumbs.tsx 🟢 — ts+react — UI wrapper Breadcrumbs — ui-core/Breadcrumbs.tsx
6️⃣6️⃣ packages/app/src/ui/sidebar.tsx 🔴 — ts+react — Sidebar (НЕТ)
6️⃣7️⃣ packages/app/src/ui/loading-spinner.tsx 🔴 — ts+react — Loading Spinner (НЕТ)
6️⃣8️⃣ packages/app/src/ui/search-bar.tsx 🔴 — ts+react — Search Bar (НЕТ)

App state и hooks
6️⃣9️⃣ packages/app/src/state/store.ts 🔴 — ts+zustand — Root store (НЕТ)
7️⃣0️⃣ packages/app/src/state/query/query-client.ts 🔴 — ts+react — React query client — store.ts (НЕТ)
7️⃣1️⃣ packages/app/src/hooks/useApi.ts 🔴 — ts+react — Hook API — lib/api-client.ts (НЕТ)
7️⃣2️⃣ packages/app/src/hooks/useToast.ts 🔴 — ts+react — Hook Toast — ui-core/useToast.ts (НЕТ)

Feature Auth
7️⃣1️⃣ packages/feature-auth/src/domain/LoginRequest.ts 🔴 — ts — DTO login (НЕТ)
7️⃣2️⃣ packages/feature-auth/src/domain/TokenPair.ts 🔴 — ts — DTO token pair (НЕТ)
7️⃣3️⃣ packages/feature-auth/src/domain/MeResponse.ts 🔴 — ts — DTO me response (НЕТ)
7️⃣4️⃣ packages/feature-auth/src/types/auth.ts 🔴 — ts — Типы auth — domain/* (НЕТ)
7️⃣5️⃣ packages/feature-auth/src/stores/auth.ts 🔴 — ts+zustand — Auth store — types/auth.ts (НЕТ)
7️⃣6️⃣ packages/feature-auth/src/effects/login.ts 🔴 — ts+effect — Login effect — api-client.ts, types/auth.ts (НЕТ)
7️⃣7️⃣ packages/feature-auth/src/effects/logout.ts 🔴 — ts+effect — Logout effect — api-client.ts (НЕТ)
7️⃣8️⃣ packages/feature-auth/src/effects/refresh.ts 🔴 — ts+effect — Refresh effect — api-client.ts (НЕТ)
7️⃣9️⃣ packages/feature-auth/src/hooks/useAuth.ts 🔴 — ts+react — Hook auth — stores/auth.ts, effects/* (НЕТ)
8️⃣0️⃣ packages/feature-auth/src/schemas.ts 🟢 — схемы на базе core-contracts (реализован полностью)

Feature Bots
8️⃣1️⃣ packages/feature-bots/src/domain/Bot.ts 🔴 — ts — Bot entity (НЕТ)
8️⃣2️⃣ packages/feature-bots/src/domain/BotTemplate.ts 🔴 — ts — Bot template (НЕТ)
8️⃣3️⃣ packages/feature-bots/src/domain/Prompt.ts 🔴 — ts — Prompt entity (НЕТ)
8️⃣4️⃣ packages/feature-bots/src/types/bots.ts 🔴 — ts — Типы bots — domain/* (НЕТ)
8️⃣5️⃣ packages/feature-bots/src/stores/bots.ts 🔴 — ts+zustand — Bots store — types/bots.ts (НЕТ)
8️⃣6️⃣ packages/feature-bots/src/effects/createBot.ts 🔴 — ts+effect — Create bot — api-client.ts, stores/bots.ts (НЕТ)
8️⃣7️⃣ packages/feature-bots/src/effects/updateBot.ts 🔴 — ts+effect — Update bot — api-client.ts, stores/bots.ts (НЕТ)
8️⃣8️⃣ packages/feature-bots/src/effects/deleteBot.ts 🔴 — ts+effect — Delete bot — api-client.ts, stores/bots.ts (НЕТ)
8️⃣9️⃣ packages/feature-bots/src/hooks/useBots.ts 🔴 — ts+react — Hook bots list — stores/bots.ts, effects/* (НЕТ)
9️⃣0️⃣ packages/feature-bots/src/hooks/useBotWizard.ts 🔴 — ts+react — Hook bot wizard — stores/bots.ts, effects/* (НЕТ)

Feature Chat
9️⃣1️⃣ packages/feature-chat/src/domain/Message.ts 🔴 — ts — Message entity (НЕТ)
9️⃣2️⃣ packages/feature-chat/src/domain/Conversation.ts 🔴 — ts — Conversation entity (НЕТ)
9️⃣3️⃣ packages/feature-chat/src/types/chat.ts 🔴 — ts — Типы chat — domain/* (НЕТ)
9️⃣4️⃣ packages/feature-chat/src/stores/chat.ts 🔴 — ts+zustand — Chat store — types/chat.ts (НЕТ)
9️⃣5️⃣ packages/feature-chat/src/effects/sendMessage.ts 🔴 — ts+effect — Send message effect — api-client.ts, stores/chat.ts (НЕТ)
9️⃣6️⃣ packages/feature-chat/src/effects/connectWebSocket.ts 🔴 — ts+effect — WebSocket connect — websocket.ts, stores/chat.ts (НЕТ)
9️⃣7️⃣ packages/feature-chat/src/hooks/useChat.ts 🔴 — ts+react — Hook chat — stores/chat.ts, effects/* (НЕТ)
9️⃣8️⃣ packages/feature-chat/src/hooks/useRealTime.ts 🔴 — ts+react — Hook real-time — effects/connectWebSocket.ts (НЕТ)

UI Features — Auth
9️⃣9️⃣ packages/ui-features/src/auth/login-form.tsx 🟢 — ts+react — Login form UI
1️⃣0️⃣0️⃣ packages/ui-features/src/auth/register-form.tsx 🟢 — ts+react — Register form UI
1️⃣0️⃣1️⃣ packages/ui-features/src/auth/WorkspaceForm.tsx 🔴 — ts+react — Workspace form UI (НЕТ)
1️⃣0️⃣2️⃣ packages/ui-features/src/auth/OnboardingFlow.tsx 🔴 — ts+react — Onboarding flow (НЕТ)
1️⃣0️⃣3️⃣ packages/ui-features/src/auth/TwoFactorAuth.tsx 🔴 — ts+react — Two factor auth UI (НЕТ)

UI Features — Bots
1️⃣0️⃣4️⃣ packages/ui-features/src/bots/BotDashboard.tsx 🔴 — ts+react — Bots dashboard (НЕТ)
1️⃣0️⃣5️⃣ packages/ui-features/src/bots/BotWizardFlow.tsx 🔴 — ts+react+effect — Bot wizard flow (НЕТ)
1️⃣0️⃣6️⃣ packages/ui-features/src/bots/BotTemplateSelector.tsx 🔴 — ts+react — Template selector (НЕТ)
1️⃣0️⃣7️⃣ packages/ui-features/src/bots/BotBasicForm.tsx 🔴 — ts+react — Bot basic form (НЕТ)
1️⃣0️⃣8️⃣ packages/ui-features/src/bots/PromptEditor.tsx 🔴 — ts+react+effect — Prompt editor (НЕТ)
1️⃣0️⃣9️⃣ packages/ui-features/src/bots/PromptBlocks.tsx 🔴 — ts+react — Prompt blocks (НЕТ)
1️⃣1️⃣0️⃣ packages/ui-features/src/bots/PromptPreview.tsx 🔴 — ts+react — Prompt preview (НЕТ)

UI Features — Chat + Pages
1️⃣1️⃣1️⃣ packages/ui-features/src/chat/ChatInterface.tsx 🔴 — ts+react+effect — Chat interface (НЕТ)
1️⃣1️⃣2️⃣ packages/ui-features/src/chat/MessageBubble.tsx 🔴 — ts+react — Message bubble (НЕТ)
1️⃣1️⃣3️⃣ packages/ui-features/src/chat/ChatInput.tsx 🔴 — ts+react — Chat input (НЕТ)
1️⃣1️⃣4️⃣ packages/ui-features/src/chat/TypingIndicator.tsx 🔴 — ts+react — Typing indicator (НЕТ)
1️⃣1️⃣5️⃣ packages/ui-features/src/chat/MessageStatus.tsx 🔴 — ts+react — Message status (НЕТ)
1️⃣1️⃣6️⃣ packages/ui-features/src/chat/Attachments.tsx 🔴 — ts+react — Attachments (НЕТ)
1️⃣1️⃣7️⃣ packages/ui-features/src/chat/ChatHistory.tsx 🔴 — ts+react — Chat history (НЕТ)

UI Features — Admin/Dashboard
1️⃣1️⃣8️⃣ packages/ui-features/src/admin/DataTable.tsx 🔴 — ts+react — Data table (НЕТ)
1️⃣1️⃣9️⃣ packages/ui-features/src/admin/Pagination.tsx 🔴 — ts+react — Pagination (НЕТ)
1️⃣2️⃣0️⃣ packages/ui-features/src/admin/FiltersPanel.tsx 🔴 — ts+react — Filters panel (НЕТ)
1️⃣2️⃣1️⃣ packages/ui-features/src/admin/StatCard.tsx 🔴 — ts+react — Stat card (НЕТ)
1️⃣2️⃣2️⃣ packages/ui-features/src/admin/Chart.tsx 🔴 — ts+react — Chart/Graph (НЕТ)
1️⃣2️⃣3️⃣ packages/ui-features/src/admin/LogsViewer.tsx 🔴 — ts+react — Logs viewer (НЕТ)
1️⃣2️⃣4️⃣ packages/ui-features/src/admin/UserRoleBadge.tsx 🔴 — ts+react — User role badge (НЕТ)

UI Features — Billing/Payments
1️⃣2️⃣5️⃣ packages/ui-features/src/billing/PricingCard.tsx 🔴 — ts+react — Pricing card (НЕТ)
1️⃣2️⃣6️⃣ packages/ui-features/src/billing/InvoiceTable.tsx 🔴 — ts+react — Invoice table (НЕТ)
1️⃣2️⃣7️⃣ packages/ui-features/src/billing/PaymentMethod.tsx 🔴 — ts+react — Payment method (НЕТ)
1️⃣2️⃣8️⃣ packages/ui-features/src/billing/BillingHistory.tsx 🔴 — ts+react — Billing history (НЕТ)
1️⃣2️⃣9️⃣ packages/ui-features/src/billing/SubscriptionStatus.tsx 🔴 — ts+react — Subscription status (НЕТ)

UI Features — PWA/Security
1️⃣3️⃣0️⃣ packages/ui-features/src/pwa/InstallPrompt.tsx 🔴 — ts+react — Install prompt (НЕТ)
1️⃣3️⃣1️⃣ packages/ui-features/src/pwa/OfflineIndicator.tsx 🔴 — ts+react — Offline indicator (НЕТ)
1️⃣3️⃣2️⃣ packages/ui-features/src/pwa/UpdateNotification.tsx 🔴 — ts+react — Update notification (НЕТ)
1️⃣3️⃣3️⃣ packages/ui-features/src/security/PermissionsTable.tsx 🔴 — ts+react — Permissions table (НЕТ)

Web Pages
1️⃣3️⃣4️⃣ apps/web/src/app/[locale]/page.tsx 🟢 — Полная главная страница с i18n и навигацией (больше чем каркас)
1️⃣3️⃣5️⃣ apps/web/src/app/[locale]/dashboard/page.tsx 🟡 — Каркас dashboard с skeleton loading, accessibility и TODO для реальных виджетов/данных
1️⃣3️⃣6️⃣ apps/web/src/app/[locale]/auth/login/page.tsx 🟡 — Каркас login страницы с ui-features композитором и TODO для реального auth flow
1️⃣3️⃣7️⃣ apps/web/src/app/[locale]/auth/register/page.tsx 🟡 — Каркас register страницы с ui-features композитором и TODO для реального auth flow
