...
2️⃣2️⃣4️⃣ effects/login.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-isolation, @livai/app/lib/api-client, types/auth, types/login.dto, domain/LoginResult, stores/auth, lib/security-pipeline, effects/login/error-mapper, effects/login/validation, effects/login/login-metadata.enricher, domain/LoginRiskAssessment, domain/DeviceInfo, schemas; (выполняет login через orchestrator с последовательностью шагов: validate-input → security-pipeline (с deterministic decision: block/require-MFA/allow до API call) → enrich-metadata → api-call (через ApiClient с validatedEffect для response schema) → update-store (через stores/auth actions: setAuthState/setSessionState, НЕ через safeSet), все шаги с isolation и timeout, обработка ошибок через error-mapper)
2️⃣2️⃣5️⃣ effects/logout.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/state/store-utils, stores/auth; (выполняет logout через orchestrator, очищает auth state через safeSet, блокировка store через setStoreLocked)
2️⃣2️⃣6️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-isolation, @livai/app/lib/api-client, types/auth, stores/auth, effects/login/error-mapper, schemas; (выполняет регистрацию через orchestrator с последовательностью шагов: validate-input → enrich-metadata → api-call (через ApiClient с validatedEffect для response schema) → update-store, все шаги с isolation и timeout, обработка ошибок через error-mapper)
2️⃣2️⃣7️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-isolation, @livai/app/state/store-utils, types/auth, stores/auth, @livai/core/policies/AuthPolicy, schemas; (обновляет access token через orchestrator с idempotency guard, валидация через validatedEffect, синхронизация store через safeSet, isolation и timeout)
2️⃣2️⃣8️⃣ lib/session-manager.ts 🔴 — ts — deps: types/auth, @livai/core/policies/AuthPolicy, domain/SessionPolicy; (auto-refresh, expiry, invalidation, session policies, concurrent limits)
2️⃣2️⃣9️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (Единый React-адаптер auth: инкапсулирует zustand+effects, предоставляет API authState/authStatus/isAuthenticated/login/logout/refresh, скрывает реализацию, точка расширения для auto-refresh, silent login, side-effects; финальный слой feature-auth, аналог useBots/useChat)
2️⃣3️⃣0️⃣ schemas.ts 🟢 — ts+zod — deps: @livai/core-contracts, domain/LoginRequest, domain/RegisterRequest, domain/RegisterResponse, domain/PasswordResetRequest, domain/PasswordResetConfirm, domain/VerifyEmailRequest, domain/VerifyPhoneRequest, domain/MfaChallengeRequest, domain/MfaSetupRequest, domain/MfaBackupCodeRequest, domain/MfaRecoveryRequest, domain/OAuthLoginRequest, domain/OAuthRegisterRequest, domain/OAuthErrorResponse, domain/AuthErrorResponse, domain/LoginRiskAssessment, domain/SessionPolicy, domain/SessionRevokeRequest, domain/AuthAuditEvent, domain/DeviceInfo, domain/TokenPair, domain/RefreshTokenRequest, domain/LogoutRequest, domain/MeResponse, domain/EmailTemplateRequest, domain/SmsTemplateRequest; (Zod схемы для валидации всех auth DTO: login/register/MFA/OAuth/security/audit/templates/errors/recovery на базе core-contracts)
