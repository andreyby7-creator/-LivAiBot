...
2️⃣1️⃣3️⃣ lib/device-fingerprint.ts 🟢 — ts+effect — deps: domain/DeviceInfo; (pure effect, без side-effects, сбор device info: userAgent, platform, screen, timezone, генерация deviceId)
...
2️⃣1️⃣7️⃣ lib/security-pipeline.ts 🟢 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/effect-timeout, @livai/app/lib/effect-utils, domain/DeviceInfo, lib/device-fingerprint, effects/login/risk-assessment, types/auth, types/auth-risk; (auth-специфичная обертка над device fingerprint и risk assessment, использует orchestrator для композиции шагов с timeout, facade pattern для единой точки входа, production guard для mandatory audit logger)
