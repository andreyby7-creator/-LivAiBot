...
Feature Auth
1️⃣8️⃣2️⃣ domain/LoginRequest.ts 🟢 — ts — deps: — ; (DTO login, только типы запроса)
1️⃣8️⃣3️⃣ domain/RegisterRequest.ts 🟢 — ts — deps: — ; (DTO register, данные для регистрации пользователя)
1️⃣8️⃣4️⃣ domain/RegisterResponse.ts 🟢 — ts — deps: domain/TokenPair, domain/MfaInfo; (DTO ответа регистрации, подтверждение создания аккаунта)
1️⃣8️⃣5️⃣ domain/PasswordResetRequest.ts 🟢 — ts — deps: — ; (DTO запрос сброса пароля, email/username)
1️⃣8️⃣6️⃣ domain/PasswordResetConfirm.ts 🟢 — ts — deps: — ; (DTO подтверждение сброса пароля, token + новый пароль)
1️⃣8️⃣7️⃣ domain/VerifyEmailRequest.ts 🟢 — ts — deps: — ; (DTO верификация email, confirmation token)
1️⃣8️⃣8️⃣ domain/VerifyPhoneRequest.ts 🟢 — ts — deps: — ; (DTO верификация телефона, SMS code)
1️⃣8️⃣9️⃣ domain/RefreshTokenRequest.ts 🟢 — ts — deps: — ; (DTO обновления токена, refresh token)
1️⃣9️⃣0️⃣ domain/LogoutRequest.ts 🟢 — ts — deps: — ; (DTO выхода из системы, опционально refresh token)
1️⃣9️⃣1️⃣ domain/DeviceInfo.ts 🟢 — ts — deps: — ; (DTO информация об устройстве для аудита)
1️⃣9️⃣2️⃣ domain/SessionRevokeRequest.ts 🟢 — ts — deps: — ; (DTO отзыв сессии, session ID)
1️⃣9️⃣3️⃣ domain/MfaChallengeRequest.ts 🟢 — ts — deps: — ; (DTO запрос MFA вызова, тип аутентификации)
1️⃣9️⃣4️⃣ domain/MfaSetupRequest.ts 🟢 — ts — deps: — ; (DTO настройки MFA, секрет и метод)
1️⃣9️⃣5️⃣ domain/MfaBackupCodeRequest.ts 🟢 — ts — deps: — ; (DTO резервных кодов MFA для recovery)
1️⃣9️⃣6️⃣ domain/OAuthLoginRequest.ts 🟢 — ts — deps: — ; (DTO OAuth login, provider token: Google/Yandex/FB/VK)
1️⃣9️⃣7️⃣ domain/OAuthRegisterRequest.ts 🟢 — ts — deps: — ; (DTO OAuth register, provider data для создания аккаунта)
1️⃣9️⃣8️⃣ domain/LoginRiskAssessment.ts 🟢 — ts — deps: — ; (DTO оценки риска логина: гео, device fingerprint, IP)
1️⃣9️⃣9️⃣ domain/SessionPolicy.ts 🟢 — ts — deps: — ; (DTO политик сессии: ограничения по IP, concurrent sessions)
2️⃣0️⃣0️⃣ domain/AuthAuditEvent.ts 🟢 — ts — deps: — ; (DTO событий аудита: client app, IP, deviceId, geo, timestamp)
2️⃣0️⃣1️⃣ domain/EmailTemplateRequest.ts 🟢 — ts — deps: — ; (DTO кастомных email шаблонов для верификации/уведомлений)
2️⃣0️⃣2️⃣ domain/SmsTemplateRequest.ts 🟢 — ts — deps: — ; (DTO кастомных SMS шаблонов для верификации)
2️⃣0️⃣3️⃣ domain/MfaRecoveryRequest.ts 🟢 — ts — deps: — ; (DTO восстановления MFA доступа при потере устройства)
2️⃣0️⃣4️⃣ domain/OAuthErrorResponse.ts 🟢 — ts — deps: — ; (DTO ошибок OAuth: invalid_token, provider_unavailable, user_denied)
2️⃣0️⃣5️⃣ domain/AuthErrorResponse.ts 🟢 — ts — deps: — ; (DTO кастомных ошибок: invalid_credentials, account_locked, rate_limited)
2️⃣0️⃣6️⃣ domain/TokenPair.ts 🟢 — ts — deps: — ; (DTO token pair: accessToken, refreshToken, expiresAt)
2️⃣0️⃣7️⃣ domain/MeResponse.ts 🟢 — ts — deps: — ; (DTO ответа /me: данные пользователя, роли, permissions)
2️⃣0️⃣8️⃣ types/auth.ts 🟢 — ts — deps: domain/LoginRequest, domain/RegisterRequest, domain/RegisterResponse, domain/PasswordResetRequest, domain/PasswordResetConfirm, domain/VerifyEmailRequest, domain/VerifyPhoneRequest, domain/MfaChallengeRequest, domain/MfaSetupRequest, domain/MfaBackupCodeRequest, domain/MfaRecoveryRequest, domain/OAuthLoginRequest, domain/OAuthRegisterRequest, domain/OAuthErrorResponse, domain/AuthErrorResponse, domain/SessionPolicy, domain/SessionRevokeRequest, domain/LoginRiskAssessment, domain/AuthAuditEvent, domain/DeviceInfo, domain/TokenPair, domain/RefreshTokenRequest, domain/LogoutRequest, domain/MeResponse, domain/EmailTemplateRequest, domain/SmsTemplateRequest; (агрегирующие типы auth: AuthState, AuthStatus, AuthError, MFA types, OAuth types, Security types, Recovery types)
2️⃣0️⃣9️⃣ types/auth-risk.ts 🟢 — ts — deps: @livai/domains, domain/LoginRiskAssessment; (auth-specific типы для risk assessment, адаптирует типы из @livai/domains для feature-auth, re-export базовых типов, auth-specific типы для RiskContext/RiskPolicy/RiskAssessmentResult, immutable readonly типы для безопасности)
2️⃣1️⃣0️⃣ lib/classification-mapper.ts 🟢 — ts — deps: @livai/domains; (маппинг classification labels из domains в auth-specific decision, strategy pattern для обработки различных labels, оптимизация производительности через pre-filtering правил, изолирует auth-специфичную логику от domain logic, используется в risk-assessment)
2️⃣1️⃣1️⃣ effects/login/login-risk-assessment.adapter.ts 🟢 — ts — deps: @livai/core, domain/DeviceInfo, domain/LoginRiskAssessment; (adapter между domain и DTO для login risk assessment, использует transformDomainToDto из @livai/core для преобразования signals в Record, защита от утечки sensitive данных через whitelist, auth-специфичная логика для LoginRiskAssessment DTO)
2️⃣1️⃣2️⃣ effects/login/risk-assessment.ts 🟢 — ts — deps: @livai/domains, lib/classification-mapper, effects/login/login-risk-assessment.adapter, domain/DeviceInfo, types/auth-risk; (composition layer, адаптирует feature-auth типы к domains API, вызывает assessClassification из domains, маппит результаты в auth-specific decision и DTO через classification-mapper, детерминированный результат, plugin pattern для расширяемости)
2️⃣1️⃣3️⃣ effects/login/device-fingerprint.ts 🟢 — ts+effect — deps: domain/DeviceInfo; (pure effect, без side-effects, сбор device info: userAgent, platform, screen, timezone, генерация deviceId)
2️⃣1️⃣4️⃣ effects/login/validation.ts 🟢 — ts — deps: domain/LoginRequest; (type guards для LoginRequest, валидация структуры и формата данных, domain-pure, deterministic, strict shape validation)
2️⃣1️⃣5️⃣ effects/login/login-metadata.enricher.ts 🟢 — ts — deps: @livai/core, domain/LoginRequest, domain/DeviceInfo, @livai/domains; (реализует ContextEnricher из @livai/core для обогащения контекста метаданными логина, использует core input-boundary для dependency-driven execution, extensible через builder registry pattern с injection, PII хеширование через injection, deterministic)
2️⃣1️⃣6️⃣ lib/error-mapper.ts 🟢 — ts — deps: @livai/app/lib/error-mapping, domain/AuthErrorResponse, domain/MfaChallengeRequest, domain/OAuthErrorResponse, domain/SessionRevokeRequest, types/auth; (трансформация API ошибок в UI-friendly AuthError без side-effects, переиспользуется для login, logout, refresh, OAuth, MFA)
2️⃣1️⃣7️⃣ lib/security-pipeline.ts 🟢 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/effect-timeout, @livai/app/lib/effect-utils, domain/DeviceInfo, effects/login/device-fingerprint, effects/login/risk-assessment, types/auth, types/auth-risk; (auth-специфичная обертка над device fingerprint и risk assessment, использует orchestrator для композиции шагов с timeout, facade pattern для единой точки входа, production guard для mandatory audit logger)
2️⃣1️⃣8️⃣ stores/auth.ts 🟢 — ts+zustand — deps: types/auth; (Auth store, чистое состояние, без effects)
2️⃣1️⃣9️⃣ types/login.dto.ts 🟢 — ts — deps: schemas; (feature-level агрегированный DTO для login-flow: LoginResponseDto как discriminated union success/mfa_required, композиция из LoginTokenPairValues и MeResponseValues, transport-oriented без domain leakage)
2️⃣2️⃣0️⃣ domain/LoginResult.ts 🟢 — ts — deps: domain/TokenPair, domain/MeResponse, domain/MfaChallengeRequest; (domain-level типы для login результата: DomainLoginResult как immutable discriminated union, Readonly типы, без transport concerns, semantic layer для оркестрации)
2️⃣2️⃣1️⃣ effects/login/login-effect.types.ts 🟢 — ts — deps: types/auth, types/auth-risk, lib/security-pipeline; (строгий DI-контракт для login-effect: порты ApiClient/LoginStorePort/SecurityPipelinePort и LoginEffectDeps/LoginEffectConfig, projection LoginSecurityResult, без runtime-логики)
2️⃣2️⃣2️⃣ effects/login/login-api.mapper.ts 🟢 — ts — deps: domain/LoginRequest, domain/LoginResult, domain/MeResponse, domain/MfaChallengeRequest, schemas, types/login.dto; (pure mapper Domain LoginRequest → LoginRequestValues и LoginResponseDto → DomainLoginResult, fail-closed через assertNever, immutability + safety boundary для dynamic Record payload)
2️⃣2️⃣3️⃣ effects/login/login-store-updater.ts 🟢 — ts — deps: effects/login/login-effect.types, effects/login/login-metadata.enricher, domain/LoginResult, lib/security-pipeline, types/auth-risk, types/auth; (единая точка применения результатов login-flow к auth/session/security store: мост между SecurityPipelineResult и DomainLoginResult, applySuccessState/applyMfaState/applyBlockedState, поддержка batchUpdate для транзакционности, fail-closed валидация дат сессии, не пересчитывает риск и не читает текущее состояние)
2️⃣2️⃣4️⃣ effects/login/login-audit.mapper.ts 🟢 — ts — deps: domain/LoginResult, schemas, types/auth; (pure mapper для преобразования результатов login-flow в AuditEventValues: mapLoginResultToAuditEvent принимает LoginResultForAudit (структурно совместим с LoginResult из login.ts) и flattened LoginAuditContext, формирует валидированный AuditEventValues через auditEventSchema, типы событий: login_success (только после успешного /me с sessionId), mfa_challenge (с userId из domainResult.challenge.userId), policy_violation (для blocked), login_failure (для error), fail-closed через проверки sessionId/userId и exhaustive switch, изолирует audit-структуру от login.ts через flattened контекст, eventId генерируется в orchestrator для детерминизма в тестах, используется через DI LoginEffectDeps.auditLogger.logAuditEvent)
2️⃣2️⃣5️⃣ effects/login.ts 🟢 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-utils, domain/LoginRequest, domain/LoginResult, schemas, types/auth, types/login.dto, effects/login/login-api.mapper, effects/login/login-effect.types, effects/login/login-metadata.enricher, effects/login/login-store-updater, effects/login/login-audit.mapper; (тонкий orchestrator login-flow: validate-input (strict Zod loginRequestSchema.strict(), generic error 'invalid_login_request' для безопасности) → security-pipeline (через SecurityPipelinePort, deterministic decision: block short-circuit до API call) → enrich-metadata (buildLoginMetadata с валидированным LoginContext) → двухфазный API-call (/v1/auth/login + /v1/auth/me через validatedEffect с loginTokenPairSchema/meResponseSchema, fail-closed: без успешного /me → login не считается успешным) → domain mapping (mapLoginResponseToDomain) → update-store (через login-store-updater: updateLoginState/applyBlockedState, атомарный batchUpdate) → audit logging (через login-audit.mapper.mapLoginResultToAuditEvent с flattened LoginAuditContext, eventId генерируется в orchestrator через generateEventId для детерминизма, login_success только после успешного /me с sessionId), hierarchical timeout (step timeouts + global hard timeout через withTimeout, loginHardTimeoutMs с дефолтом 60_000), concurrency control (cancel_previous/ignore/serialize с правильной очисткой inFlight/currentController в finally), все ошибки через injected errorMapper.map с sanitized messages, инварианты: нет бизнес-логики, нет прямых Date.now() (используется ClockPort), все side-effects через DI-порты)
2️⃣2️⃣6️⃣ effects/logout.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/state/store-utils, stores/auth; (выполняет logout через orchestrator, очищает auth state через safeSet, блокировка store через setStoreLocked)
2️⃣2️⃣7️⃣ effects/register.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-utils, domain/RegisterRequest, domain/RegisterResponse, schemas, types/auth, lib/error-mapper (через DI), effects/register/register-effect.types (DI-порты), effects/register/register-api.mapper, effects/register/register-store-updater; (orchestrator register-flow: validate-input (strict Zod registerRequestSchema.strict()) → enrich-metadata (опционально, без security-pipeline) → api-call (через validatedEffect с registerResponseSchema) → domain mapping → update-store (через register-store-updater, атомарный batchUpdate), hierarchical timeout (step timeout + global hard timeout), все ошибки через injected errorMapper.map с sanitized messages, инварианты: нет бизнес-логики, все side-effects через DI-порты)
2️⃣2️⃣8️⃣ effects/refresh.ts 🔴 — ts+effect — deps: @livai/app/lib/orchestrator, @livai/app/lib/schema-validated-effect, @livai/app/lib/effect-timeout, @livai/app/lib/effect-isolation, @livai/app/state/store-utils, types/auth, stores/auth, @livai/core/policies/AuthPolicy, schemas; (обновляет access token через orchestrator с idempotency guard, валидация через validatedEffect, синхронизация store через safeSet, isolation и timeout)
2️⃣2️⃣9️⃣ lib/session-manager.ts 🔴 — ts — deps: types/auth, @livai/core/policies/AuthPolicy, domain/SessionPolicy; (auto-refresh, expiry, invalidation, session policies, concurrent limits)
2️⃣3️⃣0️⃣ hooks/useAuth.ts 🔴 — ts+react — deps: stores/auth, effects/login, effects/logout, effects/refresh, types/auth; (Единый React-адаптер auth: инкапсулирует zustand+effects, предоставляет API authState/authStatus/isAuthenticated/login/logout/refresh, скрывает реализацию, точка расширения для auto-refresh, silent login, side-effects; финальный слой feature-auth, аналог useBots/useChat)
2️⃣3️⃣1️⃣ schemas.ts 🟢 — ts+zod — deps: @livai/core-contracts, domain/LoginRequest, domain/RegisterRequest, domain/RegisterResponse, domain/PasswordResetRequest, domain/PasswordResetConfirm, domain/VerifyEmailRequest, domain/VerifyPhoneRequest, domain/MfaChallengeRequest, domain/MfaSetupRequest, domain/MfaBackupCodeRequest, domain/MfaRecoveryRequest, domain/OAuthLoginRequest, domain/OAuthRegisterRequest, domain/OAuthErrorResponse, domain/AuthErrorResponse, domain/LoginRiskAssessment, domain/SessionPolicy, domain/SessionRevokeRequest, domain/AuthAuditEvent, domain/DeviceInfo, domain/TokenPair, domain/RefreshTokenRequest, domain/LogoutRequest, domain/MeResponse, domain/EmailTemplateRequest, domain/SmsTemplateRequest; (Zod схемы для валидации всех auth DTO: login/register/MFA/OAuth/security/audit/templates/errors/recovery на базе core-contracts)
