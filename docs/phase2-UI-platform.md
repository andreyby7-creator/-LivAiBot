...
- 🟢 `shared/operation-lifecycle.helper.ts` — ts+effect — deps: @livai/core/effect, shared/bots-store.port, lib/error-mapper, shared/audit.port — helper `createOperationLifecycleHelper` для композиции lifecycle шагов loading → run → success/failure с DI store transitions, нормализацией ошибок через `mapBotErrorToUI` и опциональными audit hooks (без бизнес-логики)
...
- ⚪ `create-bot-from-template.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, lib/_, schemas/index, domain/BotTemplate — оркестратор создания бота из шаблона (U3: hooks audit/telemetry через порт)
