...
- 🟢 `create/create-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core, types/bot-commands, types/bots, domain/Bot, lib/error-mapper — DI-контракты и конфигурация create-bot-effect: input (`workspaceId`, `operationId`, `request`) + ports/policies/error-mapping + optional lifecycle overrides (`operationHandlers`) для orchestrator'ов create-flow
...
- 🟢 `create/create-bot.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/Bot, domain/BotTemplate, types/bots, shared/api-client.port — pure/deterministic helpers для create-like flow: `buildDraftBotId`, `buildCreateBotRequestBody`, `buildActorUserContext`, а также fail-closed permission/policy pre-check функции `checkCreatePermissionsOrThrow` и `checkCreatePolicyOrThrow` (без IO и без зависимости от runtime-времени; draft-id строится детерминированно из входных данных)
- 🟢 `create-bot-from-template.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/bots-store.port, shared/api-client.port, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotTemplate, lib/bot-errors — оркестратор создания бота из шаблона на новом lifecycle-контракте: deterministic pre-flight + `runOperation` с `idempotencyKey`, pipeline-side финализацией и best-effort hooks/audit side-effects
- ⚪ `create-custom-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, lib/_, schemas/index — оркестратор создания кастомного бота (U4: hooks audit/telemetry через порт)
- ⚪ `update/update-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper — DI-контракты и конфигурация update-bot-effect
- ⚪ `update/update-bot.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/Bot, types/bots, shared/api-client.port — planned pure/deterministic helpers для update-bot flow: payload/context builders + permission/policy pre-check решения без IO/времени
- ⚪ `update/update-bot-api.mapper.ts` — ts — deps: domain/UpdateBotRequest, domain/Bot, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа update-bot API
...
- ⚪ `update/update-bot-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий bot_updated/bot_update_failed
- ⚪ `update-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, update/update-bot-effect.types, update/update-bot-api.mapper, update/update-bot-store-updater, update/update-bot-audit.mapper, lib/_, schemas/index — оркестратор update-bot-flow (hooks audit/telemetry через порт)
- ⚪ `update/update-instruction-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, lib/instruction-builder — DI-контракты и конфигурация update-instruction-effect
- ⚪ `update/update-instruction.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/BotVersion, types/bots, lib/instruction-builder, shared/api-client.port — planned pure/deterministic helpers для update-instruction flow: request builders/version guards + policy/permission pre-check без IO
- ⚪ `update/update-instruction-api.mapper.ts` — ts — deps: domain/UpdateInstructionRequest, domain/BotVersion, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа update-instruction API (версионирование)
...
- ⚪ `update/update-instruction-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий instruction_updated/instruction_update_failed
- ⚪ `update-instruction.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, update/update-instruction-effect.types, update/update-instruction-api.mapper, update/update-instruction-store-updater, update/update-instruction-audit.mapper, lib/version-manager, lib/_, schemas/index — оркестратор update-instruction-flow (U5: hooks audit/telemetry через порт)
- ⚪ `delete/delete-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, @livai/core/policies/BotPolicy, @livai/core/policies/BotPermissions, types/bots, lib/error-mapper — DI-контракты и конфигурация delete-bot-effect
- ⚪ `delete/delete-bot.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/Bot, types/bots — planned pure/deterministic helpers для delete-flow: policy/permission pre-check и deterministic context builders без IO
- ⚪ `delete/delete-bot-api.mapper.ts` — ts — deps: domain/*, schemas/index, shared/mapper.contracts — маппинг запроса/ответа delete-bot API
...
- ⚪ `delete/delete-bot-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий bot_deleted/bot_delete_failed
- ⚪ `delete-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, delete/delete-bot-effect.types, delete/delete-bot-api.mapper, delete/delete-bot-store-updater, delete/delete-bot-audit.mapper, lib/_, schemas/index — оркестратор delete-bot-flow (hooks audit/telemetry через порт)
- ⚪ `publish/publish-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper — DI-контракты и конфигурация publish-bot-effect
- ⚪ `publish/publish-bot.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/Publishing, types/bots — planned pure/deterministic helpers для publish-flow: readiness/policy pre-check и request/context builders без IO
- ⚪ `publish/publish-bot-api.mapper.ts` — ts — deps: domain/PublishBotRequest, domain/Publishing, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа publish-bot API
...
- ⚪ `publish/publish-bot-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий bot_published/bot_publish_failed
- ⚪ `publish-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, publish/publish-bot-effect.types, publish/publish-bot-api.mapper, publish/publish-bot-store-updater, publish/publish-bot-audit.mapper, lib/_, schemas/index — оркестратор publish-bot-flow (U10: hooks audit/telemetry через порт)
- ⚪ `pause-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, lifecycle/pause-bot-effect.types, lifecycle/pause-bot-api.mapper, lifecycle/pause-bot-store-updater, lifecycle/pause-bot-audit.mapper, lib/_, schemas/index — оркестратор pause-bot-flow (hooks audit/telemetry через порт)
- ⚪ `resume-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, lifecycle/resume-bot-effect.types, lifecycle/resume-bot-api.mapper, lifecycle/resume-bot-store-updater, lifecycle/resume-bot-audit.mapper, lib/_, schemas/index — оркестратор resume-bot-flow (hooks audit/telemetry через порт)
- ⚪ `archive-bot.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, lifecycle/archive-bot-effect.types, lifecycle/archive-bot-api.mapper, lifecycle/archive-bot-store-updater, lifecycle/archive-bot-audit.mapper, lib/_, schemas/index — оркестратор archive-bot-flow (hooks audit/telemetry через порт)
- ⚪ `multi-agent/multi-agent-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, lib/multi-agent-validator — DI-контракты и конфигурация multi-agent-effect
- ⚪ `multi-agent/multi-agent.helpers.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/MultiAgentSchema, lib/multi-agent-validator, types/bots — planned pure/deterministic helpers для multi-agent flow: schema/build/guard + policy pre-check без IO
- ⚪ `multi-agent/multi-agent-api.mapper.ts` — ts — deps: domain/MultiAgentSchema, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа multi-agent API
...
- ⚪ `multi-agent/multi-agent-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий multi_agent_updated/multi_agent_update_failed
- ⚪ `manage-multi-agent.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/bots-store.port, shared/api-client.port, multi-agent/multi-agent-effect.types, multi-agent/multi-agent-api.mapper, multi-agent/multi-agent-store-updater, multi-agent/multi-agent-audit.mapper, lib/_, schemas/index — оркестратор manage-multi-agent-flow (U5.1: hooks audit/telemetry через порт)
- ⚪ `test/test-bot-effect.types.ts` — ts — deps: shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, types/bots, lib/error-mapper — DI-контракты и конфигурация test-bot-effect
- ⚪ `test/test-bot.helpers.ts` — ts — deps: contracts/BotErrorResponse, lib/bot-errors, domain/TestBotRequest, types/bots — planned pure/deterministic helpers для test-bot flow: simulator request builders + deterministic validation/pre-check без IO
- ⚪ `test/test-bot-api.mapper.ts` — ts — deps: domain/TestBotRequest, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа test-bot API (симулятор чата)
- ⚪ `test/test-bot-audit.mapper.ts` — ts — deps: shared/audit.port, schemas/index, types/bots — построение audit-событий bot_tested/test_failed
- ⚪ `test-bot-simulator.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/api-client.port, test/test-bot-effect.types, test/test-bot-api.mapper, test/test-bot-audit.mapper, lib/_, schemas/index — оркестратор test-bot-simulator-flow (U10: hooks audit/telemetry через порт)
- ⚪ `templates/templates-effect.types.ts` — ts — deps: shared/api-client.port, shared/audit.port, shared/operation-lifecycle.helper, types/bots, lib/error-mapper — DI-контракты и конфигурация templates-effect
- ⚪ `templates/templates.helpers.ts` — ts — deps: contracts/BotErrorResponse, lib/bot-errors, domain/BotTemplate, types/bots — planned pure/deterministic helpers для templates flow: query/filter builders и fail-closed guards без IO
- ⚪ `templates/templates-api.mapper.ts` — ts — deps: domain/BotTemplate, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа templates API (каталог шаблонов)
- ⚪ `get-templates.ts` — ts+effect — deps: shared/operation-lifecycle.helper, shared/audit.port, shared/api-client.port, templates/templates-effect.types, templates/templates-api.mapper, lib/_, schemas/index — оркестратор get-templates-flow (hooks audit/telemetry через порт)
