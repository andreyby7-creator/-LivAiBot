...
- 🟢 `create-bot-from-template.ts` — ts+effect — deps: @livai/core/effect, shared/api-client.port, shared/operation-id-fingerprint, shared/pure-guards, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotTemplate, schemas/index, types/bot-commands, types/bots (+ через `CreateBotEffectConfig`: shared/operation-lifecycle, shared/lifecycle/*, shared/bots-store.port) — шаблон: pre-flight (permission/policy, draft `botId`), **детерминированный `operationId`** (соль `operationIdSalt.createBotFromTemplate`), `buildFallbackBotId`, `lifecycleHelper.runOperation` / `idempotencyKey`, hooks/audit через lifecycle
- 🟢 `create-custom-bot.ts` — ts+effect — deps: @livai/core/effect, shared/api-client.port, shared/operation-id-fingerprint, shared/pure-guards, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotSettings, schemas/index, types/bot-commands, types/bots, `create-bot-from-template` (типы Clock/eventId/deps) (+ через config: lifecycle/store как у from-template) — кастомный create-flow: policy `create_custom`, **детерминированный `operationId`** (соль v2 + stable JSON для `settings`), тот же fallback `botId` и audit-mapper, что и from-template
