...
- 🟢 `contracts/OperationState.ts` — ts — deps: none — boundary-типы lifecycle операций: `OperationLoading` / `OperationSuccess` / `OperationError` + универсальный `OperationState<T, E, Op>` (loading → success/error) без зависимости от feature-level `BotsState`
- 🟢 `contracts/BotsStoreContract.ts` — ts — deps: types/bots, contracts/OperationState — boundary-интенты для `bots-store` из effects/store-updaters: атомарный `batchUpdate` контракт (discriminated union) + `botsStoreContractVersion` для совместимости/эволюции
...
- 🟢 `bot-errors.ts` — ts — deps: contracts/BotErrorResponse, types/bots, domain/BotRetry — канонические метаданные `BotErrorCode` + фабрики/нормализация `BotErrorResponse` (anti-drift), фабрика доменной `BotError` + safe cause в `context.details`, retryable строго из BotRetryPolicy
- 🟢 `error-mapper.ts` — ts — deps: contracts/BotErrorResponse, types/bots, domain/BotRetry, lib/bot-errors — детерминированный DI rule-engine: boundary/unknown → `BotError` (через `mapBotErrorToUI` + канонизация `normalizeBotError`), retryable строго из BotRetryPolicy, `string` сохраняется в `context.details.cause`, плюс lazy memoization fallback/rules
...
- 🟢 `helpers/batch-update.ts` — ts — deps: contracts/BotsStoreBatchUpdate, types/bots — pure batch reducers (`applyBotsStoreBatchUpdates` и set-helpers) для применения batch-intents без Zustand
- 🟢 `bots.ts` — ts — deps: zustand, @livai/core, types/bots, stores/helpers/batch-update — чистый Zustand-store `BotsStore` (state + sync transitions) без side-effects; SSR-safe factory `createBotsStore`, `createInitialBotsStoreState` и typed actions для обновления `bots.entities`/`bots.operations`
...
- 🟢 `shared/bots-store.port.ts` — ts — deps: stores/bots, types/bots — port-контракт `BotsStorePort` + adapter для доступа к bots-store из эффектов без зависимости от Zustand; `batchUpdate` (discriminated union) + re-entrancy guard через `withStoreLock` (adapter-level, не меж-эффектный concurrency lock)
- 🟢 `shared/audit.port.ts` — ts — deps: lib/bot-audit — port-контракт `BotAuditPort` + adapter `createBotAuditPortAdapter` (`BotAuditSink` → `emit`) и `createNoopBotAuditPort` для DI observability в bot-эффектах без зависимости от конкретного sink (console/SIEM/http/kafka)
- 🟢 `shared/lifecycle/contracts.ts` — ts — deps: @livai/core/effect, @livai/core-contracts/types, types/bots — базовые lifecycle-контракты слоя execution: `OperationContext`, `ExecutionState`, `IdempotencyRecord/IdempotencyPort`, `LifecycleHooks`, `LifecycleAuditMapping`, включая semantic invariants для reconcile/takeover и bounded-retry конфигурацию
- 🟢 `shared/lifecycle/error.ts` — ts+effect — deps: shared/lifecycle/contracts, shared/audit.port, lib/error-mapper, types/bots, @livai/core/effect — единый error-boundary (`withErrorBoundary`): normalizing unknown→`BotError`, best-effort error-path side-effects (`hooks.onError`, failure audit emit) и safe finalization через `IdempotencyPort.fail` в `aborted` state без нарушения deterministic error propagation
- 🟢 `shared/lifecycle/pipeline.ts` — ts+effect — deps: shared/lifecycle/contracts, shared/bots-store.port, shared/audit.port, types/bots, @livai/core/effect — execution-pipeline слоя 3: idempotency guard/reconcile, strict order `execute → map → persist → finalize → side-effects`, bounded retry для persist и запрет partial persist при `mappingResult.ok=false`
- 🟢 `shared/operation-lifecycle.ts` — ts+effect — deps: @livai/core/effect, shared/bots-store.port, shared/audit.port, shared/lifecycle/contracts, shared/lifecycle/error, shared/lifecycle/pipeline, lib/error-mapper — публичный orchestration/glue-слой `createOperationLifecycleHelper` (compat API `runOperation`) с композицией `withErrorBoundary(...) + executeLifecyclePipeline(...)`, DI idempotency/clock и deterministic lifecycle order без бизнес-ветвлений
- 🟢 `shared/pure-guards.ts` — ts — deps: @livai/core, contracts/BotErrorResponse, lib/bot-errors, domain/Bot, types/bots — общий pure-слой: `GuardErrorDetails` / `ExtendedPolicyReason`, дефолтные и DI `*ErrorCodeResolver`, `buildGuardErrorContext`, `assertActorContextForPolicyOrThrow` + `evaluatePolicyDecision` + `throwPolicyDeniedOrReturn`, `checkPermissionsOrThrow` / `checkPolicyOrThrow(actorUser)`; permission coarse / policy fine — задокументировано в модуле; без IO/времени
- 🟢 `shared/operation-id-fingerprint.ts` — ts — deps: `fast-json-stable-stringify`, types/bot-commands — SSOT для детерминированных `operationId` (`op_<hex>`): FNV-1a 32-bit, `stableJsonFingerprint` (canonical JSON), `buildOperationIdSource` / `buildOperationIdSourceWithStableJson`, реестр солей `operationIdSalt` по эффектам; не crypto, не для секретов/PII (см. докблок); `fnv1a32` также для audit redaction surrogate в `create-bot-audit.mapper`
- 🟢 `shared/orchestrator-runtime.ts` — ts — deps: none — общий runtime-слой оркестраторов: `ClockPort` (`now()`), `EventIdGeneratorPort` (`next()`), `AuditMetaPort` + `createAuditMetaPort` (только делегирование в clock/eventId, без `Date.now`/`Math.random`), `wrapBestEffortHook` (hooks не ломают основной flow)
- 🟢 `shared/mapper.contracts.ts` — ts — deps: none — shared contracts mapper-слоя: `ParseIssue` + `MAPPER_ERROR_CODE_PARSING_JSON_INVALID` для унифицированной типизации и кодов schema/shape ошибок в API mapper'ах
- 🟢 `create/create-bot.helpers.ts` — ts — deps: @livai/core, domain/Bot, domain/BotTemplate, lib/bot-errors, shared/api-client.port, shared/pure-guards — pure/deterministic helpers: `buildDraftBotId` (ошибка формата → `BotErrorResponse` `BOT_DRAFT_ID_INVALID`), `buildCreateBotRequestBody` (frozen copy `settings`), `createCreateHelpers(PureGuardsBundle)` → `CreateBotLikeHelpers` с явными `action: BotAction` / `BotPolicyAction` (без захардкоженных default resolver’ов; multi-tenant через DI guards)
- 🟢 `create/create-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core, types/bot-commands, types/bots, domain/Bot, lib/error-mapper, create/create-bot.helpers — DI-контракты и конфигурация create-bot-effect: input (`workspaceId`, `operationId`, `request`) + ports/policies/error-mapping + `createBotLikeHelpers` (composition root: `createCreateHelpers` + `PureGuardsBundle`) + optional lifecycle overrides (`operationHandlers`) для orchestrator'ов create-flow
- 🟢 `create/create-bot-api.mapper.ts` — ts — deps: create/create-bot-effect.types, shared/api-client.port, shared/bots-api.mappers, shared/mapper.contracts, schemas/index, lib/bot-errors, contracts/BotErrorResponse, types/bots — pure: `CreateBotEffectInput` → `BotApiCreateBotInput` (fail-closed `createBotRequestSchema`) + `BotResponseTransport` → `BotInfo` через `mapBotResponseToBotInfo`
- 🟢 `create/create-bot-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — pure sink: batch-intents → `bots.entities` upsert + `operations.create` success (без `getState`/IO)
- 🟢 `create/create-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots-initial, types/bots, domain/Bot, shared/operation-id-fingerprint — audit `bot_created` (success/failure через `audit.context.result`); redaction id через `fnv1a32` из fingerprint-модуля; fail-closed `botAuditEventSchema`
- 🟢 `create-bot-from-template.ts` — ts+effect — deps: @livai/core/effect, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/pure-guards, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotTemplate, schemas/index, types/bot-commands, types/bots (+ через `CreateBotEffectConfig`: shared/operation-lifecycle, shared/lifecycle/*, shared/bots-store.port) — шаблон: pre-flight (permission/policy, draft `botId`), **детерминированный `operationId`** (соль `operationIdSalt.createBotFromTemplate`), `buildFallbackBotId`, `lifecycleHelper.runOperation` / `idempotencyKey`, hooks/audit через lifecycle
- 🟢 `create-custom-bot.ts` — ts+effect — deps: @livai/core/effect, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/pure-guards, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotSettings, schemas/index, types/bot-commands, types/bots, `create-bot-from-template` (типы Clock/eventId/deps) (+ через config: lifecycle/store как у from-template) — кастомный create-flow: policy `create_custom`, **детерминированный `operationId`** (соль v2 + stable JSON для `settings`), тот же fallback `botId` и audit-mapper, что и from-template
- ⚪ `update/update-bot.helpers.ts` — ts — deps: @livai/core, domain/Bot, types/bots, shared/api-client.port, shared/pure-guards — planned: payload/context builders + permission/policy pre-check через shared/pure-guards; явные `BotAction` / `BotPolicyAction`; по аналогии с `createCreateHelpers` — DI guards с composition root (без IO)
- ⚪ `update/update-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, update/update-bot.helpers — DI-контракты и конфигурация update-bot-effect
- ⚪ `update/update-bot-api.mapper.ts` — ts — deps: update/update-bot-effect.types, domain/UpdateBotRequest, domain/Bot, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа update-bot API
- ⚪ `update/update-bot-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — обновление store при успешном обновлении бота
- ⚪ `update/update-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots-initial, types/bots, domain/Bot — построение audit-событий bot_updated/bot_update_failed
- ⚪ `update-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, update/update-bot-effect.types, update/update-bot-api.mapper, update/update-bot-store-updater, update/update-bot-audit.mapper, update/update-bot.helpers, lib/_, schemas/index — оркестратор update-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; hooks audit/telemetry через порт)
- ⚪ `update/update-instruction.helpers.ts` — ts — deps: @livai/core, domain/BotVersion, types/bots, lib/instruction-builder, shared/api-client.port, shared/pure-guards — planned: request builders/version guards + permission/policy pre-check через shared/pure-guards (без IO)
- ⚪ `update/update-instruction-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, lib/instruction-builder, update/update-instruction.helpers — DI-контракты и конфигурация update-instruction-effect
- ⚪ `update/update-instruction-api.mapper.ts` — ts — deps: update/update-instruction-effect.types, domain/UpdateInstructionRequest, domain/BotVersion, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа update-instruction API (версионирование)
- ⚪ `update/update-instruction-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, lib/version-manager, types/bots — обновление store и создание новой версии при обновлении инструкции
- ⚪ `update/update-instruction-audit.mapper.ts` — ts — deps: schemas/index, types/bots, domain/Bot — построение audit-событий instruction_updated/instruction_update_failed
- ⚪ `update-instruction.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, update/update-instruction-effect.types, update/update-instruction-api.mapper, update/update-instruction-store-updater, update/update-instruction-audit.mapper, update/update-instruction.helpers, lib/version-manager, lib/_, schemas/index — оркестратор update-instruction-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; U5: hooks audit/telemetry через порт)
- ⚪ `delete/delete-bot.helpers.ts` — ts — deps: @livai/core, domain/Bot, types/bots, shared/pure-guards — planned: deterministic context builders + policy/permission pre-check через shared/pure-guards; явные действия/resolver’ы с composition root (без IO)
- ⚪ `delete/delete-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, @livai/core/policies/BotPermissions, types/bots, lib/error-mapper, delete/delete-bot.helpers — DI-контракты и конфигурация delete-bot-effect
- ⚪ `delete/delete-bot-api.mapper.ts` — ts — deps: delete/delete-bot-effect.types, domain/*, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа delete-bot API
- ⚪ `delete/delete-bot-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — удаление бота из store при успешном удалении
- ⚪ `delete/delete-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots-initial, types/bots, domain/Bot — построение audit-событий bot_deleted/bot_delete_failed
- ⚪ `delete-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, delete/delete-bot-effect.types, delete/delete-bot-api.mapper, delete/delete-bot-store-updater, delete/delete-bot-audit.mapper, delete/delete-bot.helpers, lib/_, schemas/index — оркестратор delete-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; hooks audit/telemetry через порт)
- ⚪ `publish/publish-bot.helpers.ts` — ts — deps: @livai/core, domain/Publishing, types/bots, shared/pure-guards — planned: readiness/pre-flight + request/context builders; policy/permission pre-check через shared/pure-guards (без IO)
- ⚪ `publish/publish-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, publish/publish-bot.helpers — DI-контракты и конфигурация publish-bot-effect
- ⚪ `publish/publish-bot-api.mapper.ts` — ts — deps: publish/publish-bot-effect.types, domain/PublishBotRequest, domain/Publishing, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа publish-bot API
- ⚪ `publish/publish-bot-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — обновление статуса бота на active при публикации
- ⚪ `publish/publish-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots, domain/Bot — построение audit-событий bot_published/bot_publish_failed
- ⚪ `publish-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, publish/publish-bot-effect.types, publish/publish-bot-api.mapper, publish/publish-bot-store-updater, publish/publish-bot-audit.mapper, publish/publish-bot.helpers, lib/_, schemas/index — оркестратор publish-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; U10: hooks audit/telemetry через порт)
- ⚪ `pause-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, pause/pause-bot-effect.types, pause/pause-bot-api.mapper, pause/pause-bot-store-updater, pause/pause-bot-audit.mapper, pause/pause-bot.helpers, lib/_, schemas/index — оркестратор pause-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; hooks audit/telemetry через порт)
- ⚪ `resume-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, resume/resume-bot-effect.types, resume/resume-bot-api.mapper, resume/resume-bot-store-updater, resume/resume-bot-audit.mapper, resume/resume-bot.helpers, lib/_, schemas/index — оркестратор resume-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; hooks audit/telemetry через порт)
- ⚪ `archive-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, archive/archive-bot-effect.types, archive/archive-bot-api.mapper, archive/archive-bot-store-updater, archive/archive-bot-audit.mapper, archive/archive-bot.helpers, lib/_, schemas/index — оркестратор archive-bot-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; hooks audit/telemetry через порт)
- ⚪ `multi-agent/multi-agent.helpers.ts` — ts — deps: @livai/core, domain/MultiAgentSchema, lib/multi-agent-validator, types/bots, shared/pure-guards — planned: schema/build/guard + policy/permission pre-check через shared/pure-guards (без IO)
- ⚪ `multi-agent/multi-agent-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, lib/multi-agent-validator, multi-agent/multi-agent.helpers — DI-контракты и конфигурация multi-agent-effect
- ⚪ `multi-agent/multi-agent-api.mapper.ts` — ts — deps: multi-agent/multi-agent-effect.types, domain/MultiAgentSchema, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа multi-agent API
- ⚪ `multi-agent/multi-agent-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — обновление multi-agent схемы в store
- ⚪ `multi-agent/multi-agent-audit.mapper.ts` — ts — deps: schemas/index, types/bots, domain/Bot — построение audit-событий multi_agent_updated/multi_agent_update_failed
- ⚪ `manage-multi-agent.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, multi-agent/multi-agent-effect.types, multi-agent/multi-agent-api.mapper, multi-agent/multi-agent-store-updater, multi-agent/multi-agent-audit.mapper, multi-agent/multi-agent.helpers, lib/_, schemas/index — оркестратор manage-multi-agent-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; U5.1: hooks audit/telemetry через порт)
- ⚪ `test/test-bot.helpers.ts` — ts — deps: domain/TestBotRequest, types/bots, lib/bot-errors, contracts/BotErrorResponse, shared/pure-guards — planned: simulator request builders + deterministic validation/fail-closed guards; permission/policy pre-check (если нужен) — через shared/pure-guards, иначе `createBotErrorResponse` для локальных validation errors (без IO)
- ⚪ `test/test-bot-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, test/test-bot.helpers — DI-контракты и конфигурация test-bot-effect
- ⚪ `test/test-bot-api.mapper.ts` — ts — deps: test/test-bot-effect.types, domain/TestBotRequest, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа test-bot API (симулятор чата)
- ⚪ `test/test-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots, domain/Bot — построение audit-событий bot_tested/test_failed
- ⚪ `test-bot-simulator.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, test/test-bot-effect.types, test/test-bot-api.mapper, test/test-bot-audit.mapper, test/test-bot.helpers, lib/_, schemas/index — оркестратор test-bot-simulator-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; U10: hooks audit/telemetry через порт)
- ⚪ `templates/templates.helpers.ts` — ts — deps: domain/BotTemplate, types/bots, lib/bot-errors, contracts/BotErrorResponse, shared/pure-guards — planned: query/filter builders и fail-closed guards; permission/policy (если появится) — через shared/pure-guards, иначе canonical errors через `lib/bot-errors` (без IO)
- ⚪ `templates/templates-effect.types.ts` — ts — deps: shared/bots-store.port, shared/api-client.port, shared/audit.port, shared/operation-lifecycle, @livai/core/policies/BotPolicy, types/bots, lib/error-mapper, templates/templates.helpers — DI-контракты и конфигурация templates-effect
- ⚪ `templates/templates-api.mapper.ts` — ts — deps: templates/templates-effect.types, domain/BotTemplate, schemas/index, shared/bots-api.mappers, shared/mapper.contracts — маппинг запроса/ответа templates API (каталог шаблонов)
- ⚪ `get-templates.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/operation-id-fingerprint, shared/orchestrator-runtime, shared/bots-api.mappers, templates/templates-effect.types, templates/templates-api.mapper, templates/templates.helpers, lib/_, schemas/index — оркестратор get-templates-flow (**детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**, где применимо к запросу; hooks audit/telemetry через порт)
