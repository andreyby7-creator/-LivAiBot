...
- **Helpers / оркестраторы:** pre-flight permission/policy и детерминированные `operationId` / fallback `botId` (draft) — в helpers (`create/*-bot.helpers` + `shared/pure-guards`, без IO); оркестраторы только glue + `runOperation`; audit/telemetry — через `shared/audit.port`.
...
- 🟢 `create/create-bot-api.mapper.ts` — ts — deps: create/create-bot-effect.types, shared/api-client.port, shared/bots-api.mappers, shared/mapper.contracts, schemas/index, lib/bot-errors, contracts/BotErrorResponse, types/bots — pure маппер create-flow: `CreateBotEffectInput` → `BotApiCreateBotInput` (fail-closed schema-валидация request) + `BotResponseTransport` → `BotInfo` через shared SSOT mapper
- 🟢 `create/create-bot-store-updater.ts` — ts — deps: contracts/OperationState, shared/bots-store.port, types/bots — pure sink для обновления bots-store: `bots.entities` + `operations.create` при success-create
- 🟢 `create/create-bot-audit.mapper.ts` — ts — deps: schemas/index, types/bots-initial, types/bots, domain/Bot — построение audit-события `bot_created` (success/failure через `audit.context.result`) с conservative redaction ключевых id (botId/workspaceId/userId) и fail-closed валидацией `botAuditEventSchema` перед формированием payload
- 🟢 `create-bot-from-template.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/bots-store.port, shared/api-client.port, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, domain/BotTemplate, lib/bot-errors — оркестратор создания бота из шаблона: pre-flight (helpers: permission/policy, draft `botId`) + **детерминированный `operationId` при отсутствии в запросе** + **`buildFallbackBotId` для failure-audit** + `runOperation`/`idempotencyKey`, pipeline-side финализация, best-effort hooks/audit (**эталон** для остальных bot-оркестраторов)
- ⚪ `create-custom-bot.ts` — ts+effect — deps: shared/operation-lifecycle, shared/lifecycle/pipeline, shared/lifecycle/error, shared/audit.port, shared/bots-store.port, shared/api-client.port, shared/bots-api.mappers, create/create-bot-effect.types, create/create-bot-api.mapper, create/create-bot-store-updater, create/create-bot-audit.mapper, create/create-bot.helpers, lib/_, schemas/index — оркестратор создания кастомного бота (тот же create-flow, что и from-template: pre-flight в helpers; **детерминированный `operationId` + fallback `botId` для failure-audit — как `create-bot-from-template`**; U4 — audit/telemetry через порт)
