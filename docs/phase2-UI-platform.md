...
- ⚪ `shared/operation-lifecycle.helper.ts` — ts+effect — deps: @livai/core/effect, shared/bots-store.port, lib/error-mapper, shared/audit.port — чистый helper для композиции effect lifecycle: loading → run → success/failure → store/audit hooks (без бизнес-логики, всё через DI)
