...
## Связка новых эффектов с `useAuth`

- Базовый React‑hook `packages/feature-auth/src/hooks/useAuth.ts` уже реализован как **тонкий фасад**:
  - возвращает только `authState` и методы эффектов: `login`, `logout`, `register`, `refresh`;
  - не содержит бизнес‑логики, policy, retry или derived‑флагов.
- При добавлении новых эффектов из этого документа (forgot/reset password, verification, MFA, OAuth, session management, profile updates):
  - **новые эффекты подключаются через DI** (расширение `UseAuthDeps` и реализация в app‑/composer‑слое);
  - **`useAuth` в feature‑слое остаётся минимальным** — без UI‑логики и derived‑флагов;
  - **derived‑флаги и удобные селекторы** (например, `isAuthenticated`, `needsVerification`) реализуются на **app‑уровне адаптеров** или через отдельные селекторы над `AuthState`, а не внутри feature‑хука.
